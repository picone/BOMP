...
$fields = array(
	...
	'visiblename' =>	array(T_ZBX_STR, O_OPT, null,			null,		'(isset({add})||isset({update}))&&!isset({inventory})'),
	...
	'interfaces' =>		array(T_ZBX_STR, O_OPT, null,			NOT_EMPTY,	'(isset({add})||isset({update}))&&!isset({inventory})', _('Agent or SNMP or JMX or IPMI interface')),
	...
	'inventory'=>       array(T_ZBX_INT, O_OPT, null,       null,           null),
	// filter
	...
	try {
		...
		if (!$create && $dbHost['flags'] == ZBX_FLAG_DISCOVERY_CREATED) {
			...
			$host = array(
				...
				'status' => hasRequest('inventory')?HOST_STATUS_NOTUSE:getRequest('status', HOST_STATUS_NOT_MONITORED),
				'description' => getRequest('description'),
		...
		if(hasRequest('inventory')){
			$_REQUEST['action']='host.list';
			unset($_REQUEST['inventory']);
		}else{
...
elseif(hasRequest('action')&&getRequest('action')=='host.list'){
	$frmForm = new CForm();
	$frmForm->cleanItems();
	$frmForm->addItem(new CDiv(array(
		new CButton('form','添加设备','redirect("hosts.php?action=host.create")')
	)));
	$hostsWidget->addPageHeader('设备列表',$frmForm);
	$sortField='name';
	$sortOrder='ASC';
    // table hosts
    $form = new CForm();
    ...
    foreach ($hosts as $host) {
        ...
        $table->addRow(array(
            ...
            SPACE,//$applications
            SPACE,//$items
			new CButton('button','添加到监控','window.location="hosts.php?form=update&hostid='.$host['hostid'].url_param('groupid').'";')
        ));
    }
    $form->addItem(array($paging, $table, $paging, get_table_header(array())));
    $hostsWidget->addItem($form);
    $hostsWidget->show();
    require_once dirname(__FILE__).'/include/page_footer.php';
}
elseif((hasRequest('action')&&getRequest('action')=='host.create')||hasRequest('inventory')){
	$hostsWidget->addPageHeader('添加设备');
	$data = array();
	if ($hostId=getRequest('hostid',0)) {
		$hostsWidget->addItem(get_header_host_table('', $_REQUEST['hostid']));
		$dbHosts = API::Host()->get(array(
	...
	$hostForm = new CView('configuration.host.inventory',$data);
	$hostsWidget->addItem($hostForm->render());
}
else {
