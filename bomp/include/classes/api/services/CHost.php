...
class CHost extends CHostGeneral {
	...
	public function get($options = array()) {
		...
		elseif (!is_null($options['templated_hosts'])) {
			$sqlParts['where']['status'] = 'h.status IN ('.HOST_STATUS_MONITORED.','.HOST_STATUS_NOT_MONITORED.','.HOST_STATUS_TEMPLATE.','.HOST_STATUS_NOTUSE.')';
		}
		...
		elseif(hasRequest('action')&&getRequest('action')=='host.list'){
			$sqlParts['where']['status'] = 'h.status='.HOST_STATUS_NOTUSE;
		}
		elseif(hasRequest('form')&&getRequest('form')=='update'){
			$sqlParts['where']['status'] = 'h.status IN ('.HOST_STATUS_MONITORED.','.HOST_STATUS_NOT_MONITORED.','.HOST_STATUS_NOTUSE.')';
		}
		else {
	...
	protected function checkInput(&$hosts, $method) {
		...
		foreach ($hosts as $host) {
			...
			if ($update) {
				...
				if (!hasRequest('inventory')&&(!isset($host['groups']) || !is_array($host['groups']) || !$host['groups'])) {
		...
		$statusValidator = new CLimitedSetValidator(array(
			'values' => array(HOST_STATUS_MONITORED, HOST_STATUS_NOT_MONITORED,HOST_STATUS_NOTUSE),
		...
		foreach ($hosts as &$host) {
			...
			if (isset($host['interfaces'])&&!hasRequest('inventory')) {
