...
//Trigger Status的表单
$triggerStatus = new CComboBox('ack_status', $filter['ackStatus'], null, array(
...
//Acknowledged Status表单的代码
$AS = new CComboBox('ack_status', $filter['ackStatus'], null, array(
    ZBX_ACK_STS_ANY => _('Any'),
    ZBX_ACK_STS_WITH_UNACK => _('With unacknowledged events'),
    ZBX_ACK_STS_WITH_LAST_UNACK => _('With last event unacknowledged')
));
...
$ageLessThan = array($statusChangeCheckBox, $statusChangeDays, SPACE, $daysSpan);

//filter by name的表单
$filterByName = new CTextBox('txt_select', $filter['txtSelect'], 40);

//show hosts in maintenance的表单
$maintenance = new CCheckBox('show_maintenance', $filter['showMaintenance'], null, 1);

//show details的表单
$details = new CCheckBox('show_details', $filter['showDetails'], null, 1);
...
//表格首行
$filterForm->addRow(_('Triggers status'), _('Acknowledge status'),_('Events'),_('Age less than'),_('Show details'),_('Show hosts in maintenance'),_('Filter by name'));

//表格第二行各表单代码
if ($config['event_ack_enable']) {
	/*$filterForm->addRow( new CComboBox('ack_status', $filter['ackStatus'], null, array(
		ZBX_ACK_STS_ANY => _('Any'),
		ZBX_ACK_STS_WITH_UNACK => _('With unacknowledged events'),
		ZBX_ACK_STS_WITH_LAST_UNACK => _('With last event unacknowledged')
	)),new CComboBox('show_triggers', $filter['showTriggers'], null, array(
        TRIGGERS_OPTION_ALL => _('Any'),
        TRIGGERS_OPTION_RECENT_PROBLEM => _('Recent problem'),
        TRIGGERS_OPTION_IN_PROBLEM => _('Problem')
    ))
    );*/
    $filterForm->addRow($triggerStatus,$AS,$eventsComboBox,$ageLessThan,$details,$maintenance,$filterByName);
}
