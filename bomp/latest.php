...
$filterTable->addRow(
	array(
		...
		new CCol(bold(_('Name').':')),
        new CCol(bold(_('Hosts').':')),
        new CCol(bold(_('Show items without data').':')),
        new CCol(bold(_('Application').':')),
        new CCol(bold(_('Show details').':')),
	)
);
...
$filterTable->addRow(
	array(
        ...
        new CCol(new CTextBox('select', $filter['select'], 40)),
		new CCol(new CMultiSelect(
			array(
				'name' => 'hostids[]',
				'objectName' => 'hosts',
		...
		new CCol(new CCheckBox('show_without_data', $filter['showWithoutData'], null, 1)),
        new CCol(
            array(
        ...
        new CCol(new CCheckBox('show_details', $filter['showDetails'], null, 1)),
	)
);
...
foreach ($applications as $appid => $dbApp) {
	...
	$table->addRow(array(
		...
		$hostName,
		new CCol(array(
				bold(iconv('GB2312','UTF8//IGNORE',$dbApp['name'])),
				' ('._n('%1$s Item', '%1$s Items',iconv('GB2312','UTF8//IGNORE',$dbApp['item_cnt'])).')'
