...
function form_actions() {
	...
	if (isset($cdef_array) && sizeof($cdef_array)) {
		if ($_POST["drp_action"] == "1") { /* delete */
			...
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下CDEF将被复制.您可以更改CDEF的标题格式.</p>
						<p><ul>$cdef_list</ul></p>
						<p><strong>标题:</strong><br>"; form_text_box("title_format", "<cdef_title> (1)", "", "255", "30", "text"); print "</p>
					</td>
				</tr>\n
				";

			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='复制CDEF'>";
		}
	}else{
		print "<tr><td bgcolor='#" . $colors["form_alternate1"]. "'><span class='textError'>您至少需要选择一个CDEF.</span></td></tr>\n";
...
function item_edit() {
	...
	html_start_box("<strong>CDEF对象</strong> [编辑: " . htmlspecialchars(db_fetch_cell("select name from cdef where id=" . $_GET["cdef_id"])) . "]", "100%", $colors["header"], "3", "center", "");
	...
	form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],0); ?>
		<td width="50%">
			<font class="textEditTitle">CDEF类型</font><br>
			选择这个CDEF对象的类型.
		</td>
		<td>
	...
	<?php form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],1); ?>
		<td width="50%">
			<font class="textEditTitle">CDEF值</font><br>
			选择这个CDEF对象的值.
		</td>
		<td>
			<?php
...
function cdef_remove() {
	...
	if ((read_config_option("deletion_verification") == "on") && (!isset($_GET["confirm"]))) {
		...
		form_confirm("您确定吗?", "您确定要删除CDEF <strong>'" . htmlspecialchars(db_fetch_cell("select name from cdef where id=" . $_GET["id"])) . "'</strong>?", htmlspecialchars("cdef.php"), htmlspecialchars("cdef.php?action=remove&id=" . $_GET["id"]));
...
function cdef_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . htmlspecialchars($cdef["name"]) . "]";
	}else{
		$header_label = "[新]";
	}

	html_start_box("<strong>CDEF</strong> $header_label", "100%", $colors["header"], "3", "center", "");
	...
	if (!empty($_GET["id"])) {
		...
		html_start_box("<strong>CDEF对象</strong>", "100%", $colors["header"], "3", "center", "cdef.php?action=item_edit&cdef_id=" . $cdef["id"]);

		print "<tr bgcolor='#" . $colors["header_panel"] . "'>";
			DrawMatrixHeaderItem("对象",$colors["header_text"],1);
			DrawMatrixHeaderItem("对象值",$colors["header_text"],1);
		...
		if (sizeof($cdef_items) > 0) {
			foreach ($cdef_items as $cdef_item) {
				form_alternate_row_color($colors["alternate"],$colors["light"],$i); $i++;
					?>
					<td>
						<a class="linkEditMain" href="<?php print htmlspecialchars("cdef.php?action=item_edit&id=" . $cdef_item["id"] . "&cdef_id=" . $cdef["id"]);?>">对象#<?php print htmlspecialchars($i);?></a>
					</td>
					<td>
					...
					<td>
						<a href="<?php print htmlspecialchars("cdef.php?action=item_movedown&id=" . $cdef_item["id"] . "&cdef_id=" . $cdef["id"]);?>"><img src="images/move_down.gif" border="0" alt="下移"></a>
						<a href="<?php print htmlspecialchars("cdef.php?action=item_moveup&id=" . $cdef_item["id"] . "&cdef_id=" . $cdef["id"]);?>"><img src="images/move_up.gif" border="0" alt="上移"></a>
					</td>
					<td align="right">
						<a href="<?php print htmlspecialchars("cdef.php?action=item_remove&id=" . $cdef_item["id"] . "&cdef_id=" . $cdef["id"]);?>"><img src="images/delete_icon.gif" style="height:10px;width:10px;" border="0" alt="删除"></a>
					</td>
				</tr>
			<?php
			}
...
function cdef() {
	...
	html_start_box("<strong>CDEF</strong>", "100%", $colors["header"], "3", "center", "cdef.php?action=edit");
			...
			<table width='100%' cellspacing='0' cellpadding='0' border='0'>
				<tr>
					<td align='left' class='textHeaderDark'>
						<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("cdef.php?filter=" . get_request_var_request("filter") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
					</td>\n
