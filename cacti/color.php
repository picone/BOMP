...
function color_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . $color["hex"] . "]";
	}else{
		$header_label = "[新]";
	}

	html_start_box("<strong>颜色</strong> $header_label", "100%", $colors["header"], "3", "center", "");
...
function color() {
	...
	html_start_box("<strong>颜色</strong>", "100%", $colors["header"], "3", "center", "color.php?action=edit");

	print "<tr bgcolor='#" . $colors["header_panel"] . "'>";
		DrawMatrixHeaderItem("Hex值",$colors["header_text"],1);
		DrawMatrixHeaderItem("颜色",$colors["header_text"],1);
		...
		foreach ($color_list as $color) {
			...
			if ($j % 4 == 1) {
				form_alternate_row_color($colors["alternate"],$colors["light"],$i); $i++;
					...
					<td align="right">
						<a href="<?php print htmlspecialchars("color.php?action=remove&id=" . $color["id"]);?>"><img src="images/delete_icon.gif" style="height:10px;width:10px;" border="0" alt="删除"></a>
					</td>
				<?php	$j=1;
			}elseif (($j % 4 == 2) || ($j % 4 == 3)) {
					?>
