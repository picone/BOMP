			...
			print "
				<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下数据输入方法将被删除.</p>
						<p><ul>$di_list</ul></p>
					</td>
				</tr>\n";
		}

		$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='删除数据输入方法'>";
	}else{
		print "<tr><td bgcolor='#" . $colors["form_alternate1"]. "'><span class='textError'>您必需至少选择一个数据输入方法.</span></td></tr>\n";
...
function field_remove() {
	...
	if ((read_config_option("deletion_verification") == "on") && (!isset($_GET["confirm"]))) {
		...
		form_confirm("您确定吗?", "你真的想要删除 <strong>'" . db_fetch_cell("select name from data_input_fields where id=" . $_GET["id"]) . "'</strong>?", htmlspecialchars("data_input.php?action=edit&id=" . $_GET["data_input_id"]), htmlspecialchars("data_input.php?action=field_remove&id=" . $_GET["id"] . "&data_input_id=" . $_GET["data_input_id"]));
...
function field_edit() {
	...
	if ((!isset($array_field_names)) && (isset($_GET["type"]) ? $_GET["type"] == "in" : false) && ($data_input["type_id"] == "1")) {
		display_custom_error_message("这个脚本看起来没有输入值,所以刚才没有添加任何数据.");
		return;
	}

	html_start_box("<strong>$header_name Fields</strong> [编辑: " . htmlspecialchars($data_input["name"]) . "]", "100%", $colors["header"], "3", "center", "");
...
function data_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . htmlspecialchars($data_input["name"]) . "]";
	}else{
		$header_label = "[新]";
	}

	html_start_box("<strong>数据输入方法</strong> $header_label", "100%", $colors["header"], "3", "center", "");
	...
	if (!empty($_GET["id"])) {
		html_start_box("<strong>输入字段</strong>", "100%", $colors["header"], "3", "center", "data_input.php?action=field_edit&type=in&data_input_id=" . $_GET["id"]);
		print "<tr bgcolor='#" . $colors["header_panel"] . "'>";
			DrawMatrixHeaderItem("名称",$colors["header_text"],1);
			DrawMatrixHeaderItem("字段顺序",$colors["header_text"],1);
			DrawMatrixHeaderItem("友好名称",$colors["header_text"],2);
		print "</tr>";
		...
		foreach ($fields as $field) {
			form_alternate_row_color($colors["alternate"],$colors["light"],$i); $i++;
				...
				<td>
					<?php print $field["sequence"]; if ($field["sequence"] == "0") { print " (未使用)"; }?>
				</td>
				<td>
		...
		<?php
			...
			print "<tr><td><em>没有输入字段</em></td></tr>";
		}
		html_end_box();

		html_start_box("<strong>输出字段</strong>", "100%", $colors["header"], "3", "center", "data_input.php?action=field_edit&type=out&data_input_id=" . $_GET["id"]);
		print "<tr bgcolor='#" . $colors["header_panel"] . "'>";
			DrawMatrixHeaderItem("名称",$colors["header_text"],1);
			DrawMatrixHeaderItem("字段顺序",$colors["header_text"],1);
			DrawMatrixHeaderItem("友好名称",$colors["header_text"],1);
			DrawMatrixHeaderItem("更新循归档",$colors["header_text"],2);
		print "</tr>";
		...
		<?php
			...
			print "<tr><td><em>没有输出字段</em></td></tr>";
		}
		html_end_box();
	}
...
function data() {
	...
	html_start_box("<strong>数据输入方法</strong>", "100%", $colors["header"], "3", "center", "data_input.php?action=edit");
				...
				<table width='100%' cellspacing='0' cellpadding='0' border='0'>
					<tr>
						<td align='left' class='textHeaderDark'>
							<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("data_input.php?filter=" . get_request_var_request("filter") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
						</td>\n
	...
	if (sizeof($data_inputs) > 0) {
		...
		print "<tr><td><em>没有数据输入方法</em></td></tr>";
	}
