			...
			print "
				<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\" 以下数据查询将被删除.</p>
						<p><ul>$dq_list</ul></p>
					</td>
				</tr>\n";
		}

		$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='删除' title='删除数据查询'>";
	}else{
		print "<tr><td bgcolor='#" . $colors["form_alternate1"]. "'><span class='textError'>您密码至少选择一个数据查询.</span></td></tr>\n";
...
function data_query_item_remove() {
	...
	if ((read_config_option("deletion_verification") == "on") && (!isset($_GET["confirm"]))) {
		...
		form_confirm("您确定吗?", "您确定要删除数据查询图形 <strong>'" . db_fetch_cell("select name from snmp_query_graph where id=" . $_GET["id"]) . "'</strong>?", htmlspecialchars("data_queries.php?action=edit&id=" . $_GET["snmp_query_id"]), htmlspecialchars("data_queries.php?action=item_remove&id=" . $_GET["id"] . "&snmp_query_id=" . $_GET["snmp_query_id"]));
...
function data_query_item_edit() {
	...
	$header_label = "[编辑: " . htmlspecialchars($snmp_query["name"]) . "]";

	html_start_box("<strong>已关联的图形/数据模板</strong> $header_label", "100%", $colors["header"], "3", "center", "");
	...
	if (!empty($snmp_query_item["id"])) {
		html_start_box("<strong>已关联的数据模版</strong>", "100%", $colors["header"], "3", "center", "");
		...
		if (sizeof($data_templates) > 0) {
			foreach ($data_templates as $data_template) {
				print "	<tr bgcolor='#" . $colors["header_panel"] . "'>
						<td><span style='color: white; font-weight: bold;'>数据模板 - " . $data_template["name"] . "</span></td>
					</tr>";
		...
		html_start_box("<strong>建议值</strong>", "100%", $colors["header"], "0", "center", "");
		...
		foreach ($data_templates as $data_template) {
			...
			print "	<tr bgcolor='#" . $colors["header_panel"] . "'>
					<td style='padding: 3px;'><span style='color: white; font-weight: bold;'>数据模板 - " . htmlspecialchars($data_template["name"]) . "</span></td>
				</tr>";
				...
				<td>
					<table cellspacing="0" cellpadding="3" border="0" width="100%">
						<tr>
							...
							<td width="220" nowrap>
								&nbsp;字段名:&nbsp;<input type="text" name="svds_<?php print $data_template["id"];?>_field" size="15">
							</td>
							<td>
								&nbsp;<input type="submit" name="svds_<?php print $data_template["id"];?>_x" value="添加" title="添加自动名称的数据源名称">
							</td>
						</tr>
					</table>
				</td>
		...
		print "	<tr bgcolor='#" . $colors["header_panel"] . "'>
				<td style='padding: 3px;'><span style='color: white; font-weight: bold;'>图形模板 - " . htmlspecialchars(db_fetch_cell("select name from graph_templates where id=" . $snmp_query_item["graph_template_id"])) . "</span></td>
			</tr>";
			...
			<td>
				<table cellspacing="0" cellpadding="3" border="0" width="100%">
					<tr>
						...
						<td width="220" nowrap>
							&nbsp;字段名:&nbsp;<input type="text" name="svg_field" size="15">
						</td>
						<td>
							&nbsp;<input type="submit" name="svg_x" value="添加" title="添加自动名称的图形标题">
						</td>
					</tr>
				</table>
			</td>
...
function data_query_edit() {
	...
	html_start_box("<strong>数据查询</strong> $header_label", "100%", $colors["header"], "3", "center", "");
	...
	if (!empty($snmp_query["id"])) {
		...
		if ((file_exists($xml_filename)) && (is_file($xml_filename))) {
			$text = "<font color='#0d7c09'><strong>定位XML文件成功</strong></font>";
			$xml_file_exists = true;
		}else{
			$text = "<font color='#ff0000'><strong>无法定位XML文件.</strong></font>";
		...
		if ($xml_file_exists == true) {
			html_start_box("<strong>已关联图形模板</strong>", "100%", $colors["header"], "3", "center", "data_queries.php?action=item_edit&snmp_query_id=" . $snmp_query["id"]);

			print "	<tr bgcolor='#" . $colors["header_panel"] . "'>
					<td><span style='color: white; font-weight: bold;'>名称</span></td>
					<td><span style='color: white; font-weight: bold;'>图形模板名称</span></td>
					<td></td>
				</tr>";
			...
			foreach ($snmp_query_graphs as $snmp_query_graph) {
				...
				print "<tr><td><em>未定义图形模板.</em></td></tr>";
			}
...
function data_query() {
	...
	html_start_box("<strong>数据查询</strong>", "100%", $colors["header"], "3", "center", "data_queries.php?action=edit");
				...
				<table width='100%' cellspacing='0' cellpadding='0' border='0'>
					<tr>
						<td align='left' class='textHeaderDark'>
							<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("data_queries.php?filter=" . get_request_var_request("filter") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
						</td>\n
	...
	$display_text = array(
		"name" => array("名字", "ASC"),
		"data_input_method" => array("数据输入方法", "ASC"));
