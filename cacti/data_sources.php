		...
		if ($_POST["drp_action"] == "1") { /* delete */
			...
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下数据源将被删除.</p>
						<p><ul>$ds_list</ul></p>";

						if (sizeof($graphs) > 0) {
							print "<tr bgcolor='#" . $colors["form_alternate1"] . "'><td class='textArea'><p class='textArea'>以下图形正在使用这些数据查询:</p>\n";
							...
							print "<br>";
							form_radio_button("delete_type", "3", "1", "保持图形不变.", "1"); print "<br>";
							form_radio_button("delete_type", "3", "2", "删除所有引用这些数据源的 <strong>图形对象</strong>.", "1"); print "<br>";
							form_radio_button("delete_type", "3", "3", "删除所有引用这些数据源的 <strong>图形</strong>.", "1"); print "<br>";
							print "</td></tr>";
						}
					print "
					</td>
				...
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='删除数据源'>";
		}elseif ($_POST["drp_action"] == "2") { /* change graph template */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>选择一个数据模板然后点击 \"继续\" 以更改以下数据源的数据模板.请小心在转换时会抑制所有警告,所以图区数据有可能会丢失.</p>
						<p><ul>$ds_list</ul></p>
						<p><strong>新数据模板:</strong><br>"; form_dropdown("data_template_id",db_fetch_assoc("select data_template.id,data_template.name from data_template order by data_template.name"),"name","id","","","0"); print "</p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='更改数据源的图形模板'>";
		}elseif ($_POST["drp_action"] == "3") { /* change host */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>为这些数据源选择一个新的设备然后点击 \"继续\"</p>
						<p><ul>$ds_list</ul></p>
						<p><strong>新主机:</strong><br>"; form_dropdown("host_id",db_fetch_assoc("select id,CONCAT_WS('',description,' (',hostname,')') as name from host order by description,hostname"),"name","id","","","0"); print "</p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='更新设备'>";
		}elseif ($_POST["drp_action"] == "4") { /* duplicate */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下数据源将被复制.您可以更改新数据源的标题格式.</p>
						<p><ul>$ds_list</ul></p>
						<p><strong>标题格式:</strong><br>"; form_text_box("title_format", "<ds_title> (1)", "", "255", "30", "text"); print "</p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='复制数据源'>";
		}elseif ($_POST["drp_action"] == "5") { /* data source -> data template */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下数据源将被转换到新的数据模板.您可以更改新数据模板的标题格式.</p>
						<p><ul>$ds_list</ul></p>
						<p><strong>标题格式:</strong><br>"; form_text_box("title_format", "<ds_title> 模版", "", "255", "30", "text"); print "</p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='转换数据源到数据模板'>";
		}elseif ($_POST["drp_action"] == "6") { /* data source enable */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下数据源将被启用.</p>
						<p><ul>$ds_list</ul></p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='启用数据源'>";
		}elseif ($_POST["drp_action"] == "7") { /* data source disable */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下数据源将被禁用.</p>
						<p><ul>$ds_list</ul></p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='禁用数据源'>";
		}elseif ($_POST["drp_action"] == "8") { /* reapply suggested data source naming */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下数据源将转换成自动名称.</p>
						<p><ul>$ds_list</ul></p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='重新应用自动名字到数据源'>";
		}else{
			...
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续'>";
		}
	}else{
		print "<tr><td bgcolor='#" . $colors["form_alternate1"]. "'><span class='textError'>您必需至少选择一个数据源.</span></td></tr>\n";
...
function data_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . htmlspecialchars($data["name"]) . "]";
	}else{
		$header_label = "[新]";
	}
	...
	if (!empty($data["data_input_id"])) {
		...
		html_start_box("<strong>自定义数据</strong> [数据输入: " . htmlspecialchars(db_fetch_cell("select name from data_input where id=" . $data["data_input_id"])) . "]", "100%", $colors["header"], "3", "center", "");
		...
		foreach ($fields as $field) {
			...
			if ((!empty($host["id"])) && (eregi('^' . VALID_HOST_FIELDS . '$', $field["type_code"]))) {
				print "<td width='50%'><strong>" . $field["name"] . "</strong> (从主机: " . $host["hostname"] . ")</td>\n";
				print "<td><em>$old_value</em></td>\n";
			}elseif (empty($can_template)) {
				print "<td width='50%'><strong>" . $field["name"] . "</strong> (从数据模板)</td>\n";
				print "<td><em>" . (empty($old_value) ? "输入为空" : $old_value) . "</em></td>\n";
			}else{
			...
			print "<tr><td><em>已选的数据输入源未输入字段</em></td></tr>";
		}
...
function ds_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . htmlspecialchars(get_data_source_title($_GET["id"])) . "]";
		...
		<table width="100%" align="center">
			<tr>
				...
				<td class="textInfo" align="right" valign="top">
					<span style="color: #c16921;">*<a href='<?php print htmlspecialchars("data_sources.php?action=ds_edit&id=" . (isset($_GET["id"]) ? $_GET["id"] : "0"));?>&debug=<?php print (isset($_SESSION["ds_debug_mode"]) ? "0" : "1");?>'><strong><?php print (isset($_SESSION["ds_debug_mode"]) ? "关闭" : "打开");?></strong>数据源调试模式.</a><br>
					<?php
						if (!empty($data_template["id"])) {
							?><span style="color: #c16921;">*<a href='<?php print htmlspecialchars("data_templates.php?action=template_edit&id=" . (isset($data_template["id"]) ? $data_template["id"] : "0"));?>'>编辑数据模板.</a><br><?php
						}
						if (!empty($_GET["host_id"]) || !empty($data_local["host_id"])) {
							?><span style="color: #c16921;">*<a href='<?php print htmlspecialchars("host.php?action=edit&id=" . (isset($_GET["host_id"]) ? $_GET["host_id"] : $data_local["host_id"]));?>'>编辑主机.</a><br><?php
						}
					?>
				</td>
			</tr>
	...
	html_start_box("<strong>选择数据模板</strong> $header_label", "100%", $colors["header"], "3", "center", "");

	$form_array = array(
		"data_template_id" => array(
			"method" => "drop_sql",
			"friendly_name" => "已选数据模板",
			//"description" => "给这个数据模板的名字.",
	...
	if (!empty($data["data_template_id"])) {
		...
		html_start_box("<strong>附加数据模板数据</strong>", "100%", $colors["header"], "3", "center", "");

		draw_nontemplated_fields_data_source($data["data_template_id"], $data["local_data_id"], $data, "|field|", "<strong>数据源字段</strong>", true, true, 0);
		draw_nontemplated_fields_data_source_item($data["data_template_id"], $template_data_rrds, "|field|_|id|", "<strong>数据源对象字段</strong>", true, true, true, 0);
		draw_nontemplated_fields_custom_data($data["id"], "value_|id|", "<strong>自字义数据</strong>", true, true, 0);
	...
	if (((isset($_GET["id"])) || (isset($_GET["new"]))) && (empty($data["data_template_id"]))) {
		html_start_box("<strong>数据源</strong>", "100%", $colors["header"], "3", "center", "");
		...
		if (!empty($_GET["view_rrd"])) {
			...
			$header_label = "[编辑: " . $rrd["data_source_name"] . "]";
		}else{
			$header_label = "";
		}
		...
		print "	<tr>
				<td bgcolor='#" . $colors["header"] . "' class='textHeaderDark'>
					<strong>数据源对象</strong> $header_label
				</td>
	...
	if ((isset($_SESSION["ds_debug_mode"])) && (isset($_GET["id"]))) {
		...
		<table width="100%" align="center">
			<tr>
				<td>
					<span class="textInfo">数据源调试</span><br>
...
function get_poller_interval($seconds) {
	if ($seconds == 0) {
		...
		return "<em>" . $seconds . " 秒</em>";
	}else if ($seconds == 60) {
		return "1 分钟";
	}else{
		return "<em>" . ($seconds / 60) . " 分钟</em>";
	}
}
...
function ds() {
	...
	<div><a href="data_sources.php?action=ds_edit&host_id="<?php echo get_request_var_request("host_id");?>"><button>添加</button></button></a></div>
	<?php

	html_start_box("<strong>数据源</strong> [主机: " . (empty($host["hostname"]) ? "未选主机" : htmlspecialchars($host["hostname"])) . "]", "100%", $colors["header"], "3", "center",'');
	?>
				...
				<tr>
					...
					<td width="1">
						<select name="template_id" onChange="applyDSFilterChange(document.form_data_sources)">
							<option value="-1"<?php if (get_request_var_request("template_id") == "-1") {?> selected<?php }?>>任意</option>
							<option value="0"<?php if (get_request_var_request("template_id") == "0") {?> selected<?php }?>>无</option>
							<?php
					...
					<td nowrap style='white-space: nowrap;'>
						...
						<input type="submit" name="clear_x" value="清除" title="标题">
					</td>
				</tr>
				<tr>
					<td width="50">
					...
					<td width="1">
						<select name="method_id" onChange="applyDSFilterChange(document.form_data_sources)">
							<option value="-1"<?php if (get_request_var_request("method_id") == "-1") {?> selected<?php }?>>任意</option>
							<option value="0"<?php if (get_request_var_request("method_id") == "0") {?> selected<?php }?>>无</option>
							<?php
				...
				<table width='100%' cellspacing='0' cellpadding='0' border='0'>
					<tr>
						<td align='left' class='textHeaderDark'>
							<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("data_sources.php?filter=" . get_request_var_request("filter") . "&host_id=" . get_request_var_request("host_id") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
						</td>\n
						<td align='center' class='textHeaderDark'>
							" . ((get_request_var_request("ds_rows")*(get_request_var_request("page")-1))+1) . " 到 " . ((($total_rows < get_request_var_request("ds_rows")) || ($total_rows < (get_request_var_request("ds_rows")*get_request_var_request("page")))) ? $total_rows : (get_request_var_request("ds_rows")*get_request_var_request("page"))) . " 行,共 $total_rows 行 [ 第 $url_page_select 页 ]
						</td>\n
						<td align='right' class='textHeaderDark'>
							<strong>"; if ((get_request_var_request("page") * get_request_var_request("ds_rows")) < $total_rows) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("data_sources.php?filter=" . get_request_var_request("filter") . "&host_id=" . get_request_var_request("host_id") . "&page=" . (get_request_var_request("page")+1)) . "'>"; } $nav .= "下一页"; if ((get_request_var_request("page") * get_request_var_request("ds_rows")) < $total_rows) { $nav .= "</a>"; } $nav .= " &gt;&gt;</strong>
						</td>\n
					</tr>
				</table>
			</td>
	...
	$display_text = array(
		"name_cache" => array("名称", "ASC"),
		"local_data_id" => array("编号","ASC"),
		"data_input_name" => array("数据输入方法", "ASC"),
		"nosort" => array("采集周期", "ASC"),
		"active" => array("活动", "ASC"),
		"data_template_name" => array("模板名称", "ASC"));
	...
	if (sizeof($data_sources) > 0) {
		foreach ($data_sources as $data_source) {
			...
			form_selectable_cell(($data_source['active'] == "on" ? "是" : "否"), $data_source["local_data_id"]);
		...
		print "<tr><td><em>无数据源</em></td></tr>";
	}

	html_end_box(false);

	/* draw the dropdown containing a list of available actions for this form */
	draw_actions_dropdown($ds_actions,'添加','data_sources.php?action=ds_edit&host_id='.get_request_var_request('host_id'));
