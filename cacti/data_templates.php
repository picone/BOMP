		...
		if ($_POST["drp_action"] == "1") { /* delete */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下数据模板将被删除.附加在这个模板的数据源将变成独立的数据源.</p>
						<p><ul>$ds_list</ul></p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='删除数据模板'>";
		}elseif ($_POST["drp_action"] == "2") { /* duplicate */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下数据模板将被复制.您可以更改新数据模板的标题格式.</p>
						<p><ul>$ds_list</ul></p>
						<p><strong>标题格式:</strong><br>"; form_text_box("title_format", "<template_title> (1)", "", "255", "30", "text"); print "</p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='复制数据模板'>";
		}
	}else{
		print "<tr><td bgcolor='#" . $colors["form_alternate1"]. "'><span class='textError'>您必需至少选择一个数据模板.</span></td></tr>\n";
...
function template_edit() {
	...
	html_start_box("<strong>数据模板</strong> " . htmlspecialchars($header_label), "100%", $colors["header"], "3", "center", "");
	...
	while (list($field_name, $field_array) = each($struct_data_source)) {
		...
		if ($field_array["flags"] == "ALWAYSTEMPLATE") {
			$form_array[$field_name]["description"] = "<em>这个字段只能使用模板.</em>";
		}else{
			$form_array[$field_name]["description"] = "";
			$form_array[$field_name]["sub_checkbox"] = array(
				"name" => "t_" . $field_name,
				"friendly_name" => "使用每数据源值(跳过这个值)",
	...
	print "	<tr>
			<td bgcolor='#" . $colors["header"] . "' class='textHeaderDark'>
				<strong>数据源对象</strong> [" . (isset($template_rrd) ? htmlspecialchars($template_rrd["data_source_name"]) : "") . "]
			</td>
			<td class='textHeaderDark' align='right' bgcolor='#" . $colors["header"] . "'>
				" . (!empty($_GET["id"]) ? "<strong><a class='linkOverDark' href='" . htmlspecialchars("data_templates.php?action=rrd_add&id=" . $_GET["id"]) . "'>新建</a>&nbsp;</strong>" : "") . "
			</td>
		</tr>\n";
	...
	if (!empty($_GET["id"])) {
		...
		html_start_box("<strong>自定义数据</strong> [数据输入: " . htmlspecialchars(db_fetch_cell("select name from data_input where id=" . $template_data["data_input_id"])) . "]", "100%", $colors["header"], "3", "center", "");
		...
		foreach ($fields as $field) {
			...
			form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],$i); ?>
				<td width="50%">
					...
					<?php form_checkbox("t_value_" . $field["data_name"], $data_input_data["t_value"], "使用每数据源值(跳过这个值)", "", "", $_GET["id"]);?>
				</td>
				<td>
					<?php form_text_box("value_" . $field["data_name"],$old_value,"","");?>
					<?php if ((eregi('^' . VALID_HOST_FIELDS . '$', $field["type_code"])) && ($data_input_data["t_value"] == "")) { print "<br><em>如果留空,值将会从主机派生.</em>\n"; } ?>
				</td>
			</tr>
			<?php
			...
			print "<tr><td><em>已选数据输入源没有输入字段</em></td></tr>";
		}
...
function template() {
	...
	html_start_box("<strong>数据模板</strong>", "100%", $colors["header"], "3", "center", "data_templates.php?action=template_edit");
			...
			<table width='100%' cellspacing='0' cellpadding='0' border='0'>
				<tr>
					<td align='left' class='textHeaderDark'>
						<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("data_templates.php?filter=" . get_request_var_request("filter") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
					</td>\n
	...
	if (sizeof($template_list) > 0) {
		foreach ($template_list as $template) {
			...
			form_selectable_cell((($template["active"] == "on") ? "启用" : "禁用"), $template["id"]);
