...
function gprint_presets_remove() {
	...
	if ((read_config_option("deletion_verification") == "on") && (!isset($_GET["confirm"]))) {
		include_once("./include/top_header.php");
		form_confirm("您确定吗?", "您确定想要删除GPRINT预设 <strong>'" . htmlspecialchars(db_fetch_cell("select name from graph_templates_gprint where id=" . $_GET["id"])) . "'</strong>? 这将影响到所有使用这个预设的图形,您在操作之前必需了解清楚!", htmlspecialchars("gprint_presets.php"), htmlspecialchars("gprint_presets.php?action=remove&id=" . $_GET["id"]));
		exit;
	}
...
function gprint_presets_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . htmlspecialchars($gprint_preset["name"]) . "]";
	}else{
		$header_label = "[新]";
	}

	html_start_box("<strong>GPRINT预设</strong> $header_label", "100%", $colors["header"], "3", "center", "");
...
function gprint_presets() {
	...
	html_start_box("<strong>GPRINT预设</strong>", "100%", $colors["header"], "3", "center", "gprint_presets.php?action=edit");

	print "	<tr bgcolor='#" . $colors["header_panel"] . "'>
			<td colspan='2' class='textSubHeaderDark'>GPRINT预设标题</td>
		</tr>";
