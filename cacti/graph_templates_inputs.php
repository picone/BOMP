...
function input_remove() {
	...
	if ((read_config_option("deletion_verification") == "on") && (!isset($_GET["confirm"]))) {
		...
		form_confirm("您确定吗?", "您确定想要删除输入对象 <strong>'" . db_fetch_cell("select name from graph_template_input where id=" . $_GET["id"]) . "'</strong>? 注意:删除这个对象将不会影响使用这个模板的图形.", htmlspecialchars("graph_templates.php?action=template_edit&id=" . $_GET["graph_template_id"]), htmlspecialchars("graph_templates_inputs.php?action=input_remove&id=" . $_GET["id"] . "&graph_template_id=" . $_GET["graph_template_id"]));
...
function input_edit() {
	...
	$header_label = "[编辑图形: " . db_fetch_cell("select name from graph_templates where id=" . $_GET["graph_template_id"]) . "]";
	...
	html_start_box("<strong>图形对象输入</strong> " . htmlspecialchars($header_label), "100%", $colors["header"], "3", "center", "");
	...
	form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],1); ?>
		<td width="50%">
			<font class="textEditTitle">已关联的图形对象</font><br>
			选择您想要允许用户输入的图形对象.
		</td>
		<td>
		<?php
		...
		foreach ($item_list as $item) {
			...
			$name = "$start_bold 对象 #" . ($i+1) . ": " . $graph_item_types{$item["graph_type_id"]} . " (" . $consolidation_functions{$item["consolidation_function_id"]} . ")$end_bold";
