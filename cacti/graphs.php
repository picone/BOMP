...
function add_tree_names_to_actions_array() {
	...
	foreach ($trees as $tree) {
		$graph_actions{"tr_" . $tree["id"]} = "添加到树 (" . $tree["name"] . ")";
	}
	}
}
...
function form_actions() {
	...
	if (isset($graph_array) && sizeof($graph_array)) {
		if ($_POST["drp_action"] == "1") { /* delete */
			...
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下图形将被删除.请注意,如果某些数据源只用在这些图形,那么也应该删除它们.</p>
						<p><ul>$graph_list</ul></p>";

						if (isset($data_sources) && sizeof($data_sources)) {
							print "<tr bgcolor='#" . $colors["form_alternate1"] . "'><td class='textArea'><p class='textArea'>以下数据源被这些图形使用:</p>\n";
							...
							print "<br>";
							form_radio_button("delete_type", "1", "1", "保留数据源不变.", "1"); print "<br>";
							form_radio_button("delete_type", "1", "2", "删除所有被这些图形使用的 <strong>数据源</strong>.", "1"); print "<br>";
							print "</td></tr>";
						}
					print "
					</td>
				...
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='删除图形'>";
		}elseif ($_POST["drp_action"] == "2") { /* change graph template */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>选择一个图形模板然后点击 \"继续\" 以更改将以下图形的图形模板.请小心在转换时会抑制所有的警告,所以图形数据有可能会丢失.</p>
						<p><ul>$graph_list</ul></p>
						<p><strong>新图形模板:</strong><br>"; form_dropdown("graph_template_id",db_fetch_assoc("select graph_templates.id,graph_templates.name from graph_templates order by name"),"name","id","","","0"); print "</p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='更改图形模板'>";
		}elseif ($_POST["drp_action"] == "3") { /* duplicate */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下图形将被复制.您可以更改新图形的标题格式.</p>
						<p><ul>$graph_list</ul></p>
						<p><strong>标题格式:</strong><br>"; form_text_box("title_format", "<graph_title> (1)", "", "255", "30", "text"); print "</p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='复制图形'>";
		}elseif ($_POST["drp_action"] == "4") { /* graph -> graph template */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下图形将被转换成图形模板.您可以更改新图形模板的标题.</p>
						<p><ul>$graph_list</ul></p>
						<p><strong>标题格式:</strong><br>"; form_text_box("title_format", "<graph_title> 模板", "", "255", "30", "text"); print "</p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='转换成图形模板'>";
		}elseif (ereg("^tr_([0-9]+)$", $_POST["drp_action"], $matches)) { /* place on tree */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下图形将被放在已选树里.</p>
						<p><ul>$graph_list</ul></p>
						<p><strong>您想要放在哪个树里:</strong><br>"; grow_dropdown_tree($matches[1], "tree_item_id", "0"); print "</p>
					</td>
				</tr>\n
				<input type='hidden' name='tree_id' value='" . $matches[1] . "'>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='把图形放在树里'>";
		}elseif ($_POST["drp_action"] == "5") { /* change host */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>为这些图形选择一个新的设备然后点击 \"继续\"</p>
						<p><ul>$graph_list</ul></p>
				...
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='更改图形关联的设备'>";
		}elseif ($_POST["drp_action"] == "6") { /* reapply suggested naming to host */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下图形将使用自动名称并重新应用到图形.</p>
						<p><ul>$graph_list</ul></p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='重新应用自动名称'>";
		}elseif ($_POST["drp_action"] == "7") { /* resize graphs */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下图形将被新指定尺寸.</p>
						<p><ul>$graph_list</ul></p>
						<p><strong>图形高度:</strong><br>"; form_text_box("graph_height", "", "", "255", "30", "text"); print "</p>
						<p><strong>图形宽度:</strong><br>"; form_text_box("graph_width", "", "", "255", "30", "text"); print "</p>
					</td>
				</tr>\n
				";

			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='调整已选图形尺寸'>";
		} else {
		...
		print "<tr><td bgcolor='#" . $colors["form_alternate1"]. "'><span class='textError'>您必需至少选择选择一个图形.</span></td></tr>\n";
...
function item() {
	...
	if (empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . htmlspecialchars(get_graph_title($_GET["id"])) . "]";
	}
	...
	html_start_box("<strong>图形对象</strong> $header_label", "100%", $colors["header"], "3", "center", $add_text);
...
function graph_diff() {
	...
	html_start_box("<strong>图形预览</strong>", "100%", $colors["header"], "3", "center", "");
	...
	foreach ($items as $item) {
		...
		if ((sizeof($graph_template_items) > sizeof($graph_items)) && ($i >= sizeof($graph_items))) {
			$mode = "add";
			$user_message = "当您点击保存,添加一个 '<strong>+</strong>' 标记 <strong>(推荐)</strong>.";
		}elseif ((sizeof($graph_template_items) < sizeof($graph_items)) && ($i >= sizeof($graph_template_items))) {
			$mode = "delete";
			$user_message = "当您点击保存,去掉一个 '<strong>-</strong>' 标记 <strong>(推荐)</strong>.";
		}
		...
		print "<td style='$action_css'><strong>对象 # " . $i . "</strong></td>\n";
	...
	<table style="background-color: #f5f5f5; border: 1px solid #aaaaaa;" width="100%" align="center">
		<tr>
			<td class="textArea">
				...
				<input type='radio' name='type' value='2'>&nbsp;当您点击保存,图形对象将保持不变 (可能导致不一致).
			</td>
		</tr>
	</table>
...
function graph_edit() {
		...
		<table width="100%" align="center">
			<tr>
				...
				<td class="textInfo" align="right" valign="top">
					<span style="color: #c16921;">*<a href='<?php print htmlspecialchars("graphs.php?action=graph_edit&id=" . (isset($_GET["id"]) ? $_GET["id"] : "0") . "&debug=" . (isset($_SESSION["graph_debug_mode"]) ? "0" : "1"));?>'><strong><?php print (isset($_SESSION["graph_debug_mode"]) ? "关闭" : "打开");?></strong>图形调试模式.</a><br>
					<?php
						if (!empty($graphs["graph_template_id"])) {
							?><span style="color: #c16921;">*<a href='<?php print htmlspecialchars("graph_templates.php?action=template_edit&id=" . (isset($graphs["graph_template_id"]) ? $graphs["graph_template_id"] : "0"));?>'>编辑图形模板.</a><br><?php
						}
						if (!empty($_GET["host_id"]) || !empty($host_id)) {
							?><span style="color: #c16921;">*<a href='<?php print htmlspecialchars("host.php?action=edit&id=" . (isset($_GET["host_id"]) ? $_GET["host_id"] : $host_id));?>'>编辑主机.</a><br><?php
						}
					?>
				</td>
			</tr>
	...
	html_start_box("<strong>选择图形模板</strong> $header_label", "100%", $colors["header"], "3", "center", "");

	$form_array = array(
		"graph_template_id" => array(
			"method" => "drop_sql",
			"friendly_name" => "已选图形模板",
			"description" => "选择一个图形模板应用到这个图形.请注意图形数据有可能会丢失.",
	...
	if (!empty($graphs["graph_template_id"])) {
		html_start_box("<strong>参考图形模板数据</strong>", "100%", $colors["header"], "3", "center", "");

		print "<form method='post' action='graphs.php'>\n";

		draw_nontemplated_fields_graph($graphs["graph_template_id"], $graphs, "|field|", "<strong>图形字段</strong>", true, true, 0);
		draw_nontemplated_fields_graph_item($graphs["graph_template_id"], $_GET["id"], "|field|_|id|", "<strong>图形对象字段</strong>", true);
		...
		<table width="100%" align="center">
			<tr>
				...
				if ((isset($_SESSION["graph_debug_mode"])) && (isset($_GET["id"]))) {
					...
					?>
					<td>
						<span class="textInfo">RRDTool命令:</span><br>
						<pre><?php print @rrdtool_function_graph($_GET["id"], 1, $graph_data_array);?></pre>
						<span class="textInfo">RRDTool结果:</span><br>
...
function graph() {
	...
	html_start_box("<strong>图形管理</strong>", "100%", $colors["header"], "3", "center", "graphs.php?action=graph_edit&host_id=" . get_request_var_request("host_id"));
				...
				<tr>
					...
					<td width="1">
						<select name="graph_rows" onChange="applyGraphsFilterChange(document.form_graph_id)">
							<option value="-1"<?php if (get_request_var_request("graph_rows") == "-1") {?> selected<?php }?>>默认</option>
							<?php
				...
				<table width='100%' cellspacing='0' cellpadding='0' border='0'>
					<tr>
						<td align='left' class='textHeaderDark'>
							<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("graphs.php?filter=" . get_request_var_request("filter") . "&host_id=" . get_request_var_request("host_id") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
						</td>\n
						<td align='center' class='textHeaderDark'>
							" . ((get_request_var_request("graph_rows")*(get_request_var_request("page")-1))+1) . " 到 " . ((($total_rows < get_request_var_request("graph_rows")) || ($total_rows < (get_request_var_request("graph_rows")*get_request_var_request("page")))) ? $total_rows : (get_request_var_request("graph_rows")*get_request_var_request("page"))) . " 行,共 $total_rows 行 [ 第 $url_page_select 页 ]
						</td>\n
	...
	$display_text = array(
		"title_cache" => array("图形标题", "ASC"),
		"local_graph_id" => array("编号", "ASC"),
		"name" => array("模板名称", "ASC"),
		"height" => array("尺寸", "ASC"));
	...
	if (sizeof($graph_list) > 0) {
		foreach ($graph_list as $graph) {
			$template_name = ((empty($graph["name"])) ? "无" : $graph["name"]);
