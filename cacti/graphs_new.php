	...
	while (list($form_type, $form_array) = each($selected_graphs_array)) {
		...
		while (list($form_id1, $form_array2) = each($form_array)) {
			...
			if ($current_form_type == "cg") {
				...
				debug_log_insert("new_graphs", "添加图形: " . get_graph_title($return_array["local_graph_id"]));
				...
				if (sizeof($return_array["local_data_id"])) { # we expect at least one data source associated
					...
					debug_log_insert("new_graphs", "错误:无已关联的数据源.请检查模板");
				}
	...
	while (list($form_type, $form_array) = each($selected_graphs_array)) {
		while (list($form_id1, $form_array2) = each($form_array)) {
			if ($form_type == "cg") {
				...
				html_start_box("<strong>添加图形来自 '" . db_fetch_cell("select name from graph_templates where id=$graph_template_id") . "'", "100%", $colors["header"], "3", "center", "");
			}elseif ($form_type == "sg") {
				while (list($form_id2, $form_array3) = each($form_array2)) {
					/* ================= input validation ================= */
				...
				/* DRAW: Data Query */
				html_start_box("<strong>添加 $num_graphs 图形" . (($num_graphs>1) ? "s" : "") . " 从 '" . db_fetch_cell("select name from snmp_query where id=$snmp_query_id") . "'", "100%", $colors["header"], "3", "center", "");
			}
			...
			array_push($num_output_fields, draw_nontemplated_fields_graph($graph_template_id, $graph_template, "g_$snmp_query_id" . "_" . $graph_template_id . "_|field|", "<strong>图形</strong> [模板: " . $graph_template["graph_template_name"] . "]", false, false, (isset($snmp_query_graph_id) ? $snmp_query_graph_id : 0)));
			array_push($num_output_fields, draw_nontemplated_fields_graph_item($graph_template_id, 0, "gi_" . $snmp_query_id . "_" . $graph_template_id . "_|id|_|field|", "<strong>图形对象</strong> [模板: " . $graph_template_name . "]", false));

			/* DRAW: Data Sources */
			if (sizeof($data_templates) > 0) {
			foreach ($data_templates as $data_template) {
				array_push($num_output_fields, draw_nontemplated_fields_data_source($data_template["data_template_id"], 0, $data_template, "d_" . $snmp_query_id . "_" . $graph_template_id . "_" . $data_template["data_template_id"] . "_|field|", "<strong>数据源</strong> [模板: " . $data_template["data_template_name"] . "]", false, false, (isset($snmp_query_graph_id) ? $snmp_query_graph_id : 0)));
				...
				array_push($num_output_fields, draw_nontemplated_fields_custom_data($data_template["id"], "c_" . $snmp_query_id . "_" . $graph_template_id . "_" . $data_template["data_template_id"] . "_|id|", "<strong>数据源</strong> [模板: " . $data_template["data_template_name"] . "]", false, false, $snmp_query_id));
			}
			}
...
function graphs() {
	...
	if (!empty($debug_log)) {
		...
		<div id='message'>
			<?php print "<table align='center' style='width:100%;background-color:#" . $colors["header"] . ";'><tr><td style='align:center;padding:3px;font-weight:bold;font-size:10pt;text-align:center;'>图形已添加</td><td style='width:1px;align:right;'><input type='button' value='清除' onClick='javascript:document.getElementById(\"message\").style.display=\"none\"' style='align=right;'></td></tr></table>";?>
	...
	<table width="100%" cellpadding="0" align="center">
		<tr>
			...
			<td width="1">
				...
				<option value="-2"<?php if ($_REQUEST["graph_type"] == "-2") {?> selected<?php }?>>全部</option>
				<option value="-1"<?php if ($_REQUEST["graph_type"] == "-1") {?> selected<?php }?>>基于图形模板</option>
				<?php
			...
			<td nowrap style='white-space: nowrap;' class="textInfo" align="center" valign="top">
				<span style="white-space: nowrap; color: #c16921;">*</span><a href="<?php print htmlspecialchars("host.php?action=edit&id=" . $_REQUEST["host_id"]);?>">编辑这个主机</a><br>
				<span style="white-space: nowrap; color: #c16921;">*</span><a href="<?php print htmlspecialchars("host.php?action=edit");?>">添加新主机</a><br>
	...
	if ($_REQUEST["graph_type"] < 0) {
		html_start_box("<strong>图形模板</strong>", "100%", $colors["header"], "3", "center", "");

		print "	<tr bgcolor='#" . $colors["header_panel"] . "'>
				<td class='textSubHeaderDark'>图形模板名称</td>
				<td width='1%' align='center' bgcolor='#819bc0' style='" . get_checkbox_style() . "'><input type='checkbox' style='margin: 0px;' name='all_cg' title='全选' onClick='SelectAll(\"cg\",this.checked);gt_update_selection_indicators();'></td>\n
			</tr>\n";
		...
		foreach ($graph_templates as $graph_template) {
			...
			print "		<td onClick='gt_select_line(" . $graph_template["graph_template_id"] . ");'>
						<span id='gt_text$query_row" . "_0'><strong>添加:</strong> " . htmlspecialchars($graph_template["graph_template_name"]) . "</span>
					</td>
					<td align='right'>
		...
		print "	<tr bgcolor='#" . (($i % 2 == 0) ? "ffffff" : $colors["light"]) . "'>
				<td colspan='2' width='60' nowrap>
					<strong>添加:</strong>&nbsp;";
					form_dropdown("cg_g", $available_graph_templates, "name", "id", "", "(根据类型选择要添加的图形)", "", "textArea");
		print "		</td>
			</tr>";
							...
							<table  cellspacing='0' cellpadding='0' width='100%' >
								<tr>
									<td class='textHeaderDark'>
										<strong>数据查询</strong> [" . $snmp_query["name"] . "]
									</td>
									<td align='right' nowrap>
										<a href='" . htmlspecialchars("graphs_new.php?action=query_reload&id=" . $snmp_query["id"] . "&host_id=" . $host["id"]) . "'><img src='images/reload_icon_small.gif' title='重新加载已关联的数据源' alt='' border='0' align='middle'></a>
									</td>
								</tr>
							</table>
						</td>
			...
			if ($xml_array != false) {
				...
				if ((strlen($_REQUEST["filter"]) == 0) ||
										...
										<table width='100%' cellspacing='0' cellpadding='0' border='0'>
											<tr>
												<td align='left' class='textHeaderDark'>
													<strong>&lt;&lt; "; if ($page > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("graphs_new.php?page" . $snmp_query["id"] . "=" . ($page-1)) . "'>"; } $nav .= "上一页"; if ($page > 1) { $nav .= "</a>"; } $nav .= "</strong>
												</td>\n
												<td align='center' class='textHeaderDark'>
													" . (($row_limit*($page-1))+1) . " 到 " . ((($total_rows < $row_limit) || ($total_rows < ($row_limit*$page))) ? $total_rows : ($row_limit*$page)) . " 行,共 $total_rows 行 [ 第 $url_page_select 页 ]
												</td>\n
												<td align='right' class='textHeaderDark'>
													<strong>"; if (($page * $row_limit) < $total_rows) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("graphs_new.php?page" . $snmp_query["id"] . "=" . ($page+1)) . "'>"; } $nav .= "下一页"; if (($page * $row_limit) < $total_rows) { $nav .= "</a>"; } $nav .= " &gt;&gt;</strong>
												</td>\n
											</tr>
										</table>
									</td>
					...
					if (!sizeof($snmp_query_indexes)) {
						print "<tr bgcolor='#" . $colors["form_alternate1"] . "'><td>这个数据查询返回了0行,或许执行这个数据查询时发生了一个错误.您可以<a href='" . htmlspecialchars("host.php?action=query_verbose&id=" . $snmp_query["id"] . "&host_id=" . $host["id"]) . "'>在调试模式运行这个数据查询</a>获取更多信息.</td></tr>\n";
					}else{
						print "	<tr bgcolor='#" . $colors["header_panel"] . "'>
								$html_dq_header
								<td width='1%' align='center' bgcolor='#819bc0' style='" . get_checkbox_style() . "'><input type='checkbox' style='margin: 0px;' name='all_" . $snmp_query["id"] . "' title='全选' onClick='SelectAll(\"sg_" . $snmp_query["id"] . "\",this.checked);dq_update_selection_indicators();'></td>\n
							</tr>\n";
					}
					...
					print "<tr bgcolor='#" . $colors["form_alternate1"] . "'><td colspan='2' style='color: red; font-size: 12px; font-weight: bold;'>搜索没有返回行.</td></tr>\n";
				}
			}else{
				print "<tr bgcolor='#" . $colors["form_alternate1"] . "'><td colspan='2' style='color: red; font-size: 12px; font-weight: bold;'>数据查询错误.</td></tr>\n";
			}
			...
			if (sizeof($data_query_graphs) == 1) {
				...
				print "	<table align='center' width='100%'>
						<tr>
							...
							<td align='right'>
								<span style='font-size: 12px; font-style: italic;'>选择一个图形类型:</span>&nbsp;
