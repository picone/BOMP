...
function add_tree_names_to_actions_array() {
	...
	if (sizeof($trees) > 0) {
		foreach ($trees as $tree) {
			$device_actions{"tr_" . $tree["id"]} = "添加到树 (" . $tree["name"] . ")";
		}
	}
}
...
function form_actions() {
	...
	if (isset($host_array) && sizeof($host_array)) {
		if ($_POST["drp_action"] == "2") { /* Enable Devices */
			...
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='启用主机'>";
			...
			print "	<tr>
					<td colspan='2' class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>要更改以下主机的可用性选项,选中需要修改的字段并输入新的值,然后点击 \"继续\".</p>
						<p><ul>" . $host_list . "</ul></p>
					</td>
					</tr>";
					$form_array = array();
			...
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='更改主机可用性选项'>";
		}elseif ($_POST["drp_action"] == "5") { /* Clear Statisitics for Selected Devices */
			print "	<tr>
					<td colspan='2' class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>要清除以下主机的除记数器,点击 \"继续\".</p>
						<p><ul>" . $host_list . "</ul></p>
					</td>
					</tr>";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='清除主机的统计信息'>";
		}elseif ($_POST["drp_action"] == "1") { /* delete */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\" 以下设备将被删除.</p>
						<p><ul>" . $host_list . "</ul></p>";
						form_radio_button("delete_type", "2", "1", "保留所有图形和数据源不变.然而数据源将被禁用.", "1"); print "<br>";
						form_radio_button("delete_type", "2", "2", "删除所有已关联的 <strong>图形</strong> 和 <strong>数据源</strong>.", "1"); print "<br>";
						print "</td></tr>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='删除主机'>";
		}elseif (ereg("^tr_([0-9]+)$", $_POST["drp_action"], $matches)) { /* place on tree */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下主机将被添加到已选的树.</p>
						<p><ul>" . $host_list . "</ul></p>
						<p><strong>您想要添加到哪个树里:</strong><br>"; grow_dropdown_tree($matches[1], "tree_item_id", "0"); print "</p>
					</td>
				</tr>\n
				<input type='hidden' name='tree_id' value='" . $matches[1] . "'>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='添加主机到树'>";
		} else {
		...
		print "<tr><td bgcolor='#" . $colors["form_alternate1"]. "'><span class='textError'>您必需至少选择一个主机.</span></td></tr>\n";
...
function host_remove() {
	...
	if ((read_config_option("deletion_verification") == "on") && (!isset($_GET["confirm"]))) {
		...
		form_confirm("您确定吗?", "您真的想要删除主机 <strong>'" . htmlspecialchars(db_fetch_cell("select description from host where id=" . $_GET["id"])) . "'</strong>?", htmlspecialchars("host.php"), htmlspecialchars("host.php?action=remove&id=" . $_GET["id"]));
...
function host_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . htmlspecialchars($host["description"]) . "]";
	}else{
		$header_label = "[新]";
	}
			...
			<tr>
				...
				<?php if (($host["availability_method"] == AVAIL_SNMP) ||
					...
					if ((($host["snmp_community"] == "") && ($host["snmp_username"] == "")) ||
						($host["snmp_version"] == 0)) {
						print "<span style='color: #ab3f1e; font-weight: bold;'>SNMP未使用</span>\n";
					}else{
						...
						if ($snmp_system == "") {
							print "<span style='color: #ff0000; font-weight: bold;'>SNMP错误</span>\n";
						}else{
							...
							print "<strong>操作系统:</strong>" . html_split_string($snmp_system) . "<br>\n";
							$days      = intval($snmp_uptime / (60*60*24*100));
							$remainder = $snmp_uptime % (60*60*24*100);
							$hours     = intval($remainder / (60*60*100));
							$remainder = $remainder % (60*60*100);
							$minutes   = intval($remainder / (60*100));
							print "<strong>运行时间:</strong> $snmp_uptime";
							print "&nbsp;($days 天, $hours 小时, $minutes 分钟)<br>\n";
							print "<strong>主机名:</strong> $snmp_hostname<br>\n";
							print "<strong>位置:</strong> $snmp_location<br>\n";
							print "<strong>联系人:</strong> $snmp_contact<br>\n";
						}
					}
					?>
					</span>
				...
				<td class="textInfo" valign="top">
					<span style="color: #c16921;">*</span><a href="<?php print htmlspecialchars("graphs_new.php?host_id=" . $host["id"]);?>">为这个主机添加图形</a><br>
					<span style="color: #c16921;">*</span><a href="<?php print htmlspecialchars("data_sources.php?host_id=" . $host["id"] . "&ds_rows=30&filter=&template_id=-1&method_id=-1&page=1");?>">数据源列表</a><br>
					<span style="color: #c16921;">*</span><a href="<?php print htmlspecialchars("graphs.php?host_id=" . $host["id"] . "&graph_rows=30&filter=&template_id=-1&page=1");?>">图形列表</a>
	...
	html_start_box("<strong>主机</strong> $header_label", "100%", $colors["header"], "3", "center", "");
	...
	if ((isset($_GET["display_dq_details"])) && (isset($_SESSION["debug_log"]["data_query"]))) {
		html_start_box("<strong>数据查询调试信息</strong>", "100%", $colors["header"], "3", "center", "");
	...
	if (!empty($host["id"])) {
		html_start_box("<strong>已关联的图形模板</strong>", "100%", $colors["header"], "3", "center", "");

		html_header(array("图形模板名称", "状态"), 2);
		...
		foreach ($selected_graph_templates as $item) {
			...
			<tr>
				...
				<td>
					<?php print (($is_being_graphed == true) ? "<span style='color: green;'>正在绘图</span> (<a href='" . htmlspecialchars("graphs.php?action=graph_edit&id=" . db_fetch_cell("select id from graph_local where graph_template_id=" . $item["id"] . " and host_id=" . $_GET["id"] . " limit 0,1")) . "'>编辑</a>)" : "<span style='color: #484848;'>没有绘图</span>");?>
				</td>
				<td align='right' nowrap>
					<a href='<?php print htmlspecialchars("host.php?action=gt_remove&id=" . $item["id"] . "&host_id=" . $_GET["id"]);?>'><img src='images/delete_icon_large.gif' title='删除图形模板关联' alt='Delete Graph Template Association' border='0' align='middle'></a>
				</td>
			</tr>
			<?php
		}
		}else{ print "<tr><td><em>没有已关联的图形模板.</em></td></tr>"; }
		...
		<tr bgcolor="#<?php print $colors["form_alternate1"];?>">
			<td colspan="4">
				<table cellspacing="0" cellpadding="1" width="100%">
					...
					<td align="right">
						&nbsp;<input type="submit" value="添加" name="add_gt_x" title="添加图形模板到主机">
					</td>
				</table>
			</td>
		</tr>
		...
		html_start_box("<strong>已关联的数据查询</strong>", "100%", $colors["header"], "3", "center", "");

		html_header(array("数据查询名称", "调试", "重新索引方法", "状态"), 2);
		...
		foreach ($selected_data_queries as $item) {
			...
			<tr>
				...
				<td>
					(<a href="<?php print htmlspecialchars("host.php?action=query_verbose&id=" . $item["id"] . "&host_id=" . $_GET["id"]);?>">详细查询</a>)
				</td>
				<td>
					<?php print $reindex_types{$item["reindex_method"]};?>
				</td>
				<td>
					<?php print (($status == "success") ? "<span style='color: green;'>成功</span>" : "<span style='color: green;'>失败</span>");?> [<?php print $num_dq_items;?> 个对象 , <?php print $num_dq_rows;?> 行]
				</td>
				<td align='right' nowrap>
					<a href='<?php print htmlspecialchars("host.php?action=query_reload&id=" . $item["id"] . "&host_id=" . $_GET["id"]);?>'><img src='images/reload_icon_small.gif' title='重新加载数据查询' alt='Reload Data Query' border='0' align='middle'></a>&nbsp;
					<a href='<?php print htmlspecialchars("host.php?action=query_remove&id=" . $item["id"] . "&host_id=" . $_GET["id"]);?>'><img src='images/delete_icon_large.gif' title='删除数据查询关联' alt='Delete Data Query Association' border='0' align='middle'></a>
				</td>
			</tr>
			<?php
		}
		}else{ print "<tr><td><em>没有已关联的数据查询.</em></td></tr>"; }
		...
		<tr bgcolor="#<?php print $colors["form_alternate1"];?>">
			<td colspan="5">
				<table cellspacing="0" cellpadding="1" width="100%">
					...
					<td align="right">
						&nbsp;<input type="submit" value="添加" name="add_dq_x" title="添加数据查询到主机">
					</td>
				</table>
			</td>
		</tr>
...
function host() {
	...
	<a href="host.php?action=edit&host_template_id=<?php echo get_request_var_request('host_template_id');?>&host_status=<?php echo get_request_var_request('host_status');?>"><button type="button">添加主机</button></a>
	<?php
				...
				<tr>
					...
					<td width="1">
						<select name="host_template_id" onChange="applyViewDeviceFilterChange(document.form_devices)">
							<option value="-1"<?php if (get_request_var_request("host_template_id") == "-1") {?> selected<?php }?>>任意</option>
							<option value="0"<?php if (get_request_var_request("host_template_id") == "0") {?> selected<?php }?>>无</option>
							<?php
					...
					<td width="1">
						<select name="host_status" onChange="applyViewDeviceFilterChange(document.form_devices)">
							<option value="-1"<?php if (get_request_var_request("host_status") == "-1") {?> selected<?php }?>>任意</option>
							<option value="-3"<?php if (get_request_var_request("host_status") == "-3") {?> selected<?php }?>>已启用</option>
							<option value="-2"<?php if (get_request_var_request("host_status") == "-2") {?> selected<?php }?>>已禁用</option>
							<option value="-4"<?php if (get_request_var_request("host_status") == "-4") {?> selected<?php }?>>不在线</option>
							<option value="3"<?php if (get_request_var_request("host_status") == "3") {?> selected<?php }?>>在线</option>
							<option value="1"<?php if (get_request_var_request("host_status") == "1") {?> selected<?php }?>>宕机</option>
							<option value="2"<?php if (get_request_var_request("host_status") == "2") {?> selected<?php }?>>正在恢复</option>
							<option value="0"<?php if (get_request_var_request("host_status") == "0") {?> selected<?php }?>>未知</option>
						</select>
					</td>
					<td nowrap style='white-space: nowrap;' width="50">
						&nbsp;搜索:&nbsp;
					...
					<td width="1">
						<select name="host_rows" onChange="applyViewDeviceFilterChange(document.form_devices)">
							<option value="-1"<?php if (get_request_var_request("host_rows") == "-1") {?> selected<?php }?>>默认</option>
							<?php
				...
				<table width='100%' cellspacing='0' cellpadding='0' border='0'>
					<tr>
						<td align='left' class='textHeaderDark'>
							<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("host.php?filter=" . get_request_var_request("filter") . "&host_template_id=" . get_request_var_request("host_template_id") . "&host_status=" . get_request_var_request("host_status") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
						</td>\n
						<td align='center' class='textHeaderDark'>
							" . ((get_request_var_request("host_rows")*(get_request_var_request("page")-1))+1) . " 到 " . ((($total_rows < read_config_option("num_rows_device")) || ($total_rows < (get_request_var_request("host_rows")*get_request_var_request("page")))) ? $total_rows : (get_request_var_request("host_rows")*get_request_var_request("page"))) . " 行,共 $total_rows 行 [ 第 $url_page_select 页 ]
						</td>\n
	...
	$display_text = array(
		...
		"status" => array("状态", "ASC"),
		"status_event_count" => array("事件数量", "ASC"),
		"hostname" => array("主机名", "ASC"),
		"cur_time" => array("当前延时(毫秒)", "DESC"),
		"avg_time" => array("平均延时(毫秒)", "DESC"),
		"availability" => array("可用性(百分比)", "ASC"));
	...
	draw_actions_dropdown($device_actions,'添加主机','host.php?action=edit&host_template_id='.get_request_var_request('host_template_id').'&host_status='.get_request_var_request('host_status'));
	print "</form>\n";
}
?>