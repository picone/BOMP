		...
		if ($_POST["drp_action"] == "1") { /* delete */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>您真的想要删除以下主机模板吗?所有关联的主机都将失去关联.</p>
						<p><ul>$host_list</ul></p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='删除主机模板'>";
		}elseif ($_POST["drp_action"] == "2") { /* duplicate */
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						<p>当您点击 \"继续\",以下主机模板将被复制.您可以更改新主机模板的标题格式.</p>
						<p><ul>$host_list</ul></p>
						<p><strong>标题:</strong><br>"; form_text_box("title_format", "<template_title> (1)", "", "255", "30", "text"); print "</p>
					</td>
				</tr>\n
				";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='复制主机模板'>";
		}
	}else{
		print "<tr><td bgcolor='#" . $colors["form_alternate1"]. "'><span class='textError'>您必需至少选择一个主机模板.</span></td></tr>\n";
...
function template_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . $host_template["name"] . "]";
	}else{
		$header_label = "[新]";
		$_GET["id"] = 0;
	}

	html_start_box("<strong>主机模板</strong> " . htmlspecialchars($header_label), "100%", $colors["header"], "3", "center", "");
		...
		<tr bgcolor="#<?php print $colors["form_alternate1"];?>">
			<td colspan="2">
				<table cellspacing="0" cellpadding="1" width="100%">
					...
					<td align="right">
						&nbsp;<input type="submit" value="添加" name="add_gt_x" title="添加图形模板到主机模板">
					</td>
				</table>
			</td>
		</tr>
		...
		<tr bgcolor="#<?php print $colors["form_alternate1"];?>">
			<td colspan="2">
				<table cellspacing="0" cellpadding="1" width="100%">
					...
					<td align="right">
						&nbsp;<input type="submit" value="添加" name="add_dq_x" title="添加数据查询到主机模板">
					</td>
				</table>
			</td>
		</tr>
...
function template() {
	...
	html_start_box("<strong>主机模板</strong>", "100%", $colors["header"], "3", "center", "host_templates.php?action=edit");
			...
			<table width='100%' cellspacing='0' cellpadding='0' border='0'>
				<tr>
					<td align='left' class='textHeaderDark'>
						<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("host_templates.php?filter=" . get_request_var_request("filter") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
					</td>\n
	...
	if (sizeof($template_list) > 0) {
		foreach ($template_list as $template) {
			...
			form_selectable_cell("<a class='linkEditMain' href='" . htmlspecialchars("host_templates.php?action=edit&id=" . $template["id"]) . "'>" . (strlen(get_request_var_request("filter")) ? eregi_replace("(" . preg_quote(get_request_var_request("filter")) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", htmlspecialchars($template["name"])) : htmlspecialchars($template["name"])) . "</a>", $template["id"]);
