...
function run_data_query($host_id, $snmp_query_id) {
	...
	debug_log_insert("data_query", "运行数据查询 [$snmp_query_id].");
...
function get_data_query_array($snmp_query_id) {
	...
	if (!file_exists($xml_file_path)) {
		debug_log_insert("data_query", "未找到数据查询XML文件'$xml_file_path'");
		return false;
	}

	debug_log_insert("data_query", "找到数据查询XML文件:'$xml_file_path'");
...
function query_script_host($host_id, $snmp_query_id) {
	...
	if ((!is_array($script_queries)) || (sizeof($script_queries) == 0)) {
		debug_log_insert("data_query", "解析XML文件发生错误.");
		return false;
	}

	debug_log_insert("data_query", "XML文件解析成功.");
	...
	debug_log_insert("data_query", "执行索引列表脚本:'$script_path'");
	...
	while (list($field_name, $field_array) = each($script_queries["fields"])) {
		if ($field_array["direction"] == "input") {
			...
			debug_log_insert("data_query", "执行脚本查询'$script_path'");
			...
			for ($i=0;($i<sizeof($script_data_array));$i++) {
				if (preg_match("/(.*)" . preg_quote($script_queries["output_delimeter"]) . "(.*)/", $script_data_array[$i], $matches)) {
					...
					debug_log_insert("data_query", "找到对象:[$field_name='$field_value'] 索引:$script_index");
				}
			}
		}
	}
...
function query_snmp_host($host_id, $snmp_query_id) {
	...
	if ($host["hostname"] == "") {
		debug_log_insert("data_query", "不可用的主机ID: $host_id");
		return false;
	}

	/* invalid xml check */
	if ((!is_array($snmp_queries)) || (sizeof($snmp_queries) == 0)) {
		debug_log_insert("data_query", "解析XML文件错误.");
		return false;
	}
	...
	debug_log_insert("data_query", "执行SNMP WALK: '" . $snmp_queries["oid_index"] . "'");

	/* no data found; get out */
	if (!$snmp_indexes) {
		debug_log_insert("data_query", "无SNMP数据返回");
		return false;
	} else {
		/* show list of indices found */
		for ($i=0; $i<sizeof($snmp_indexes); $i++) {
			debug_log_insert("data_query", "找到索引,OID: '" . $snmp_indexes[$i]["oid"] . "' 值: '" . $snmp_indexes[$i]["value"] . "'");
		}
	}
	...
	while (list($field_name, $field_array) = each($snmp_queries["fields"])) {
		if ((!isset($field_array["oid"])) && ($field_array["source"] == "index")) {
			...
			debug_log_insert("data_query", "定位输入字段 '$field_name' [get]");
			...
			if ($field_array["source"] == "value") {
				for ($i=0; $i<sizeof($snmp_indexes); $i++) {
					...
					debug_log_insert("data_query", "执行SNMP GET: '$oid' [value='$value']");
			...
			debug_log_insert("data_query", "定位输入字段 '$field_name' [WALK]");
			...
			debug_log_insert("data_query", "执行SNMP WALK: '" . $field_array["oid"] . "'");
			...
			if ($field_array["source"] == "value") {
				for ($i=0; $i<sizeof($snmp_data); $i++) {
					...
					debug_log_insert("data_query", "找到对象 [$field_name='" . $snmp_data[$i]["value"] . "'] 索引: $snmp_index [来自值]");
				...
				for ($i=0; $i<sizeof($snmp_data); $i++) {
					...
					debug_log_insert("data_query", "找到对象 [$field_name='$value'] 索引: $snmp_index [从正则表达式的OID解析]");
				...
				for ($i=0; $i<sizeof($snmp_data); $i++) {
					...
					debug_log_insert("data_query", "找到对象 [$field_name='$value'] 索引: $snmp_index [from regexp value parse]");
