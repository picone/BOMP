							...
							<table width="100%" cellpadding="0" cellspacing="0">
								<tr>
									<td bgcolor="#<?php print $background_color;?>" class="textHeaderDark"><?php print $title;?></td>
										<?php if ($add_text != "") {?><td class="textHeaderDark" align="right" bgcolor="#<?php print $colors["header"];?>"><strong><a class="linkOverDark" href="<?php print htmlspecialchars($add_text);?>">添加</a>&nbsp;</strong></td><?php }?>
								</tr>
							</table>
						</td>
					</tr><?php }?>
				...
				if ($print) {
					print "<tr bgcolor='#a9b7cb'>
						<td colspan='3' class='textHeaderDark'>
							<strong>图形模板:</strong> " . htmlspecialchars($graph["graph_template_name"]) . "
						</td>
					</tr>";
				}
					...
					<table align='center' cellpadding='0'>
						<tr>
							...
							<td valign='top' style='align: left; padding: 3px;' class='noprint'>
								<a href='<?php print htmlspecialchars("graph.php?action=zoom&local_graph_id=" . $graph["local_graph_id"] . "&rra_id=0&". $extra_url_args);?>'><img src='images/graph_zoom.gif' border='0' alt='Zoom Graph' title='图形缩放' style='padding: 3px;'></a><br>
								<a href='<?php print htmlspecialchars("graph_xport.php?local_graph_id=" . $graph["local_graph_id"] . "&rra_id=0&" . $extra_url_args);?>'><img src='images/graph_query.png' border='0' alt='CSV Export' title='导出逗号分隔符' style='padding: 3px;'></a><br>
								<a href='<?php print htmlspecialchars("graph.php?action=properties&local_graph_id=" . $graph["local_graph_id"] . "&rra_id=0&" . $extra_url_args);?>'><img src='images/graph_properties.gif' border='0' alt='Graph Source/Properties' title='图形源/属性' style='padding: 3px;'></a><br>
								<?php api_plugin_hook('graph_buttons', array('hook' => 'graphs_thumbnails', 'local_graph_id' => $graph['local_graph_id'], 'rra' =>  0, 'view_type' => 'view')); ?>
								<a href='#page_top'><img src='images/graph_page_top.gif' border='0' alt='Page Top' title='回到页首' style='padding: 3px;'></a><br>
							</td>
						</tr>
					</table>
				</td>
		...
		foreach ($graph_array as $graph) {
			if (isset($graph["graph_template_name"])) {
				...
				if ($print) {
					...
					print "<tr style='background-color:#a9b7cb;'>
						<td style='background-color:#a9b7cb;' colspan='" . read_graph_config_option("num_columns") . "' class='textHeaderDark'>
							<strong>图形模板:</strong> " . $graph["graph_template_name"] . "
						</td>
					</tr>";
					$i = 0;
				}
				...
				if ($print) {
					...
					print "<tr style='background-color:#a9b7cb;'>
							<td style='background-color:#a9b7cb;' colspan='" . read_graph_config_option("num_columns") . "' class='textHeaderDark'><strong>数据查询:</strong> " . $graph["data_query_name"] . "</td>
						</tr>";
					$i = 0;
				}
				...
				<table align='center' cellpadding='0'>
					<tr>
						...
						<td valign='top' style='align: left; padding: 3px;'>
							<a href='<?php print htmlspecialchars($config['url_path'] . "graph.php?action=zoom&local_graph_id=" . $graph["local_graph_id"] . "&rra_id=0&" . $extra_url_args);?>'><img src='<?php print $config['url_path'];?>images/graph_zoom.gif' border='0' alt='Zoom Graph' title='图形缩放' style='padding: 3px;'></a><br>
							<a href='<?php print htmlspecialchars($config['url_path'] . "graph_xport.php?local_graph_id=" . $graph["local_graph_id"] . "&rra_id=0&" . $extra_url_args);?>'><img src='<?php print $config['url_path'];?>images/graph_query.png' border='0' alt='CSV Export' title='导出逗号分隔符文件' style='padding: 3px;'></a><br>
							<a href='<?php print htmlspecialchars($config['url_path'] . "graph.php?action=properties&local_graph_id=" . $graph["local_graph_id"] . "&rra_id=0&" . $extra_url_args);?>'><img src='<?php print $config['url_path'];?>images/graph_properties.gif' border='0' alt='Graph Source/Properties' title='图形源/属性' style='padding: 3px;'></a><br>
				...
				<tr>
					<td align='left' class='textHeaderDark' width='15%'>
						<?php if ($current_page > 1) {
							print "<strong><a class='linkOverDark' href='" . htmlspecialchars(str_replace("<PAGE>", ($current_page-1), $nav_url)) . "'> &lt;&lt; 上一页</a></strong>";
						} ?>
					</td>
					<td align='center' class='textHeaderDark' width='70%'>
						<?php print (($rows_per_page*($current_page-1))+1);?> 到 <?php print ((($total_rows < $rows_per_page) || ($total_rows < ($rows_per_page*$current_page))) ? $total_rows : ($rows_per_page*$current_page));?> 行,共 <?php print $total_rows;?> 行
					</td>
					<td align='right' class='textHeaderDark' width='15%'>
						<?php if (($current_page * $rows_per_page) < $total_rows) {
							print "<strong><a class='linkOverDark' href='" . htmlspecialchars(str_replace("<PAGE>", ($current_page+1), $nav_url)) . "'> 下一页 &gt;&gt; </a></strong>";
						} ?>
					</td>
				</tr>
			</table>
...
function draw_graph_items_list($item_list, $filename, $url_data, $disable_controls) {
	...
	print "<tr bgcolor='#" . $colors["header_panel"] . "'>";
		...
		DrawMatrixHeaderItem("CF类型",$colors["header_text"],1);
...
function draw_actions_dropdown($actions_array,$text='',$href='') {
	global $config;
	?>
	<table align='center' width='100%'>
		<tr>

			<?php if($text!=''){echo '<td valign="top"><a href="',$href,'"><button type="button">',$text,'</button></a></td>';}?>
			<td align='right'>
				选择一个操作:
				<?php form_dropdown("drp_action",$actions_array,"","","1","","");?>
			</td>
			<td width='1' align='right'>
				<input type='submit' value='确定' title='执行'>
			</td>
		</tr>
	</table>
