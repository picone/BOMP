...
function get_colored_device_status($disabled, $status) {
	...
	if ($disabled) {
		return "<span style='color: #$disabled_color'>禁用</span>";
	}else{
		switch ($status) {
			case HOST_DOWN:
				return "<span style='color: #" . $status_colors[HOST_DOWN] . "'>宕机</span>"; break;
			case HOST_RECOVERING:
				return "<span style='color: #" . $status_colors[HOST_RECOVERING] . "'>正在恢复</span>"; break;
			case HOST_UP:
				return "<span style='color: #" . $status_colors[HOST_UP] . "'>在线</span>"; break;
			case HOST_ERROR:
				return "<span style='color: #" . $status_colors[HOST_ERROR] . "'>错误</span>"; break;
			default:
				return "<span style='color: #0000ff'>未知</span>"; break;
		}
	}
}
