...
class Net_Ping
{
		...
		if ($this->host["hostname"]) {
			...
			if ($this->is_ipaddress($this->host["hostname"])) {
				...
				if (!$this->is_ipaddress($host_ip)) {
					cacti_log("WARNING: ICMP检测失败: gethostbyname failed for " . $this->host["hostname"]);
					$this->response = "ICMP检测失败: gethostbyname failed for " . $this->host["hostname"];
					return false;
				}
			}
				...
				while (1) {
					if ($error <> "bad_seq") {
						...
						if (substr_count($host_ip,":") > 0) {
							if (defined("AF_INET6")) {
								...
								$this->ping_response = "PHP版本不支持IPv6";
								$this->ping_status   = "down";
					...
					case 1:
						...
						if ($sqn != $this->sqn) {
							...
							$this->ping_response = "ICMP检测成功 (" . $this->time*1000 . " ms)";
			...
			if (strtolower(PHP_OS) != "winnt") {
				...
				if ($position > 0) {
					...
					$this->ping_response = "ICMP检测成功(" . $results[1] . " 毫秒)";
				...
				if ($position > 0) {
					...
					$this->ping_response = "ICMP检测成功(" . $this->ping_status . " 毫秒)";
	...
	function ping_udp() {
		...
		if ($this->host["hostname"]) {
			...
			if ($this->is_ipaddress($this->host["hostname"])) {
				...
				if (!$this->is_ipaddress($host_ip)) {
					cacti_log("WARNING: UDP检测出错: gethostbyname failed for " . $this->host["hostname"]);
					$this->response = "UDP检测出错: gethostbyname failed for " . $this->host["hostname"];
					return false;
				}
			}
			...
			if (substr_count($host_ip,":") > 0) {
				if (defined("AF_INET6")) {
					...
					$this->ping_response = "PHP版本不支持IPv6";
					$this->ping_status   = "down";
					cacti_log("WARNING: 找到IPv6主机,PHP版本不支持IPv6");
			...
			while (1) {
				if ($retry_count >= $this->retries) {
					$this->status = "down";
					if ($error == "timeout") {
						$this->ping_response = "UDP检测超时";
					}else{
						$this->ping_response = "UDP检测被拒绝";
					}
					$this->close_socket();
				...
				case 1:
					...
					/* set the return message */
					$this->ping_status = $this->time * 1000;
					$this->ping_response = "UDP检测成功(" . $this->time*1000 . "毫秒)";
	...
	function ping_tcp() {
		...
		if ($this->host["hostname"]) {
			...
			if ($this->is_ipaddress($this->host["hostname"])) {
				...
				if (!$this->is_ipaddress($host_ip)) {
					cacti_log("WARNING: TCP检测出错: gethostbyname failed for " . $this->host["hostname"]);
					$this->response = "TCP检测出错: gethostbyname failed for " . $this->host["hostname"];
					return false;
				}
			}
			...
			if (substr_count($host_ip,":") > 0) {
				if (defined("AF_INET6")) {
					...
					$this->ping_response = "PHP程序不支持IPv6";
					$this->ping_status   = "down";
					cacti_log("WARNING: 找到IPv6主机,但PHP版本不支持IPv6");
			...
			while (1) {
				...
				case 2:
					...
					if (($this->time*1000) <= $this->timeout) {
						$this->ping_response = "TCP检测连接被拒绝(" . $this->time*1000 . "毫秒)";
						$this->ping_status   = $this->time*1000;
					}
				...
				case 1:
					...
					if (($this->time*1000) <= $this->timeout) {
						$this->ping_response = "TCP检测成功 (" . $this->time*1000 . " ms)";
						$this->ping_status   = $this->time*1000;
					}
	...
	function ping($avail_method = AVAIL_SNMP_AND_PING, $ping_type = PING_ICMP, $timeout=500, $retries=3) {
		...
		$this->ping_status   = "down";
		$this->ping_response = "由于设置,未执行检测.";
		$this->snmp_status   = "down";
		$this->snmp_response = "由于设置或检测结果,SNMP未执行.";
