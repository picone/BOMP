...
print '<center>最后刷新时间: ' . date('G时i分s秒', time()) . '(服务器时间)</center><br>';

// If the sound is on, and a host is down, show the button to silence it
if ($sound && $host_down)
	print "<center><form action='" . $config['url_path'] . "plugins/monitor/monitor.php' method=POST><input type=hidden name=sound value='off'><input type='submit' name='button_nosound_x' value='静音'></center></form><br>";

if (!$sound && $host_down)
	print '<center><b>报警声音被关闭!</b></center><br>';
...
if ($host_down) {
	...
	$down_host_message = '';
	$down_host_message .= '<br><br><center><h2>主机宕机消息</h2><table cellspacing=0 cellpadding=1 bgcolor=black><tr><td><table bgcolor=white width="100%">';
...
if ($monitor_legend) {
	print "<br><br><br><center><table cellpadding=1 cellspacing=0 bgcolor='#000000'><tr><td>&nbsp;<font color='#FFFFFF'><b>图例</b></font></td></tr><tr><td bgcolor='#000000'>\n";
	...
	if ($thold) {
		...
		print "<tr valign=top align=center><td width='25%'>正常</td><td width='25%'>正在<br>恢复</td><td width='25%'>超过<br>阈值</td><td width='25%'>宕机</td></tr>";
	} else {
		...
		print "<tr valign=top align=center><td width='33%'>正常</td><td width='33%'>正在<br>恢复</td><td width='33%'>宕机</td></tr>";
	}
...
function render_group_by_tree() {
	...
	if (sizeof($heirarchy) > 0) {
		$result .= render_branch($heirarchy, "其它");
	}
...
function render_host($host) {
	...
	$icolorsdisplay = array('未知', '<font color=red><b>宕机</b></font>', '正在恢复', '正常', '超过阈值', '<font color=red><b>宕机</b></font> (已静音)');
	...
	$title = "<table cellpadding=0 cellspacing=0 width='250px'><tr><td colspan=2><b>$name</b></td></tr><tr valign=top><td>状态:</td><td>$sdisplay</td></tr><tr valign=top><td>IP地址:</td><td>$hostname</td></tr><tr valign=top><td>延时:</td><td>$ptime 毫秒</td></tr><tr valign=top><td>最后宕机:</td><td>$d</td></tr><tr valign=top><td>可用性:&nbsp;&nbsp;</td><td>$avail%</td></tr></table>";
...
function monitor_print_host_downtime ($status_fail_date) {
	...
	if ($dt_d > 0 ) {
		$dt .= $dt_d . "天" . $dt_h . "小时" . $dt_m . "分" . $dt_s . "秒";
	} else if ($dt_h > 0 ) {
		$dt .= $dt_h . "小时" . $dt_m . "分" . $dt_s . "秒";
	} else if ($dt_m > 0 ) {
		$dt .= $dt_m . "分" . $dt_s . "秒";
	} else {
		$dt .= $dt_s . "秒";
