...
class Mailer {
	...
	function Mailer($config = array(), $headers = array()) {
		...
		$this->Config["Mail"]["CharSet"] = "GB2312";
	...
	function _send_mail() {
		...
		if ($this->Config["Mail"]["Type"] == "PHP") {
			...
			if (($smtp_sock = @fsockopen($this->Config["Mail"]["SMTP_Host"], $this->Config["Mail"]["SMTP_Port"], $errno, $errstr, 1)) === false) {
				$this->Error = "无法连接到SMTP服务器 '" . $this->Config["Mail"]["SMTP_Host"] . "': (" . $errno . ") " . $errstr;
				return false;
			}
			...
			if (substr($smtp_response,0,3) != "220") {
				$this->Error = "SMTP服务器返回错误: " . $smtp_response;
				fclose($smtp_sock);
				return false;
			}
			...
			if ((! empty($this->Config["Mail"]["SMTP_Username"])) && (! empty($this->Config["Mail"]["SMTP_Password"]))) {
				...
				if (substr($smtp_response,0,3) != "334") {
					$this->Error = "SMTP可能不支持验证: " . $smtp_response;
					fclose($smtp_sock);
					return false;
				}
				...
				$smtp_response = fgets($smtp_sock, 4096);
				if (substr($smtp_response,0,3) != "334") {
					$this->Error = "SMTP验证失败: " . $smtp_response;
					fclose($smtp_sock);
					return false;
				}
			...
			$smtp_response = fgets($smtp_sock, 4096);
			if (substr($smtp_response,0,3) != "250") {
				$this->Error = "SMTP服务器不接受发件人地址: " . $smtp_response;	
				fclose($smtp_sock);
				return false;
			}
			...
			foreach ($to as $item) {
				...
				$smtp_response = fgets($smtp_sock, 4096);
				if (substr($smtp_response,0,3) != "250") {
					$this->Error = "SMTP服务器不接受收件人地址: " . $smtp_response;
					fclose($smtp_sock);
					return false;
				}
			}
			...
			if (substr($smtp_response,0,3) != "354") {
				$this->Error = "SMTP服务器不接受data命令:" . $smtp_response;	
				fclose($smtp_sock);
				return false;
			}
			...
			$smtp_response = fgets($smtp_sock,4096);
			if (substr($smtp_response,0,3) != "250") {
				$this->Error = "SMTP发送邮件时出错: " . $smtp_response;	
				fclose($smtp_sock);
				return false;
			}
			...
			if (substr($smtp_response,0,3) != "221") {
				$this->Error = "SMTP服务器不接受quid命令: " . $smtp_response;	
				fclose($smtp_sock);
				return false;
			}
