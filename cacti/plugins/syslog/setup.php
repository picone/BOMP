...
function syslog_config_settings() {
	...
	$settings["visual"]["syslog_header"] = array(
		"friendly_name" => "Syslog设置",
		"method" => "spacer"
	);
	$settings["visual"]["num_rows_syslog"] = array(
		"friendly_name" => "每页行数",
		"description" => "在Syslog页面,每页显示多少行事件.",
		"method" => "textbox",
		"default" => "30",
		"max_length" => "3"
	);
	...
	$temp = array(
		...
		"syslog_enabled" => array(
			"friendly_name" => "启用Syslog",
			"description" => "如果选中此项，Syslog消息将被写入数据库，并启用报警和报告功能，如果不启用Syslog，所有的Syslog消息将被记录在临时数据库里，如果您长期不使用Syslog功能，请停止并禁用rsyslog服务.",
			"method" => "checkbox",
			"default" => "on"
		),
		"syslog_domains" => array(
			"friendly_name" => "按域名清除",
			"description" => "填写您希望按域名自动删除的Syslog<br>例如： 'Cacti.com, zhengfeng.net'",
			"method" => "textbox",
			"default" => "",
			"size" => 80,
			"max_length" => 255,
		...
		"syslog_refresh" => array(
			"friendly_name" => "刷新周期",
			"description" => "页面自动刷新的周期，单位：秒",
			"method" => "drop_array",
			"default" => "300",
			"array" => $syslog_refresh
		),
		"syslog_maxrecords" => array(
			"friendly_name" => "最大报告数量",
			"description" => "您希望显示在报告中的最大数量。该功能用来限制报警邮件大小",
			"method" => "drop_array",
			"default" => "100",
			"array" => array("20" => "20条", "40" => "40条", "60" => "60条", "100" => "100条", "200" => "200条", "400" => "400条")
		),
		"syslog_retention" => array(
			"friendly_name" => "Syslog保留",
			"description" => "Syslog日志保留多少天",
			"method" => "drop_array",
			"default" => "30",
			"array" => $syslog_retentions
		),
		"syslog_alert_retention" => array(
			"friendly_name" => "Syslog报警保留",
			"description" => "Syslog报警日志保留多少天",
			"method" => "drop_array",
			"default" => "30",
			"array" => $syslog_alert_retentions
		),
		"syslog_html" => array(
			"friendly_name" => "基于HTML的邮件",
			"description" => "如果选中此项，所有的邮件将以HTML格式发送，否则邮件将以纯文本方式发送",
			"method" => "checkbox",
			"default" => ""
		),
//		"syslog_ticket_command" => array(
//			"friendly_name" => "Command for Opening Tickets",
//			"description" => "这个命令将被执行用作桌面支持，命令需要以下几个选项： <b>--alert-name</b>, <b>--severity</b>, <b>--hostlist</b>, <b>--message</b>。hostlist：主机列表
...
//		),
		"syslog_email" => array(
			"friendly_name" => "邮件发件人地址",
			"description" => "Syslog报警邮件将使用该地址发送",
			"method" => "textbox",
			"max_length" => 128,
		),
		"syslog_emailname" => array(
			"friendly_name" => "发件人姓名",
			"description" => "Syslog报警邮件将使用该发件人发送",
			"method" => "textbox",
			"max_length" => 128,
		),
		"syslog_bgcolors_header" => array(
			"friendly_name" => "事件背景颜色",
			"method" => "spacer",
		),
...
function syslog_config_arrays () {
	...
	$syslog_freqs = array('86400' => '最后一天', '604800' => '最后一周');
	...
	foreach ($menu as $temp => $temp2 ) {
		...
		if ($temp == 'Import/Export') {
			$menu2["Syslog Settings"]['plugins/syslog/syslog_alerts.php'] = "报警规则";
			$menu2["Syslog Settings"]['plugins/syslog/syslog_removal.php'] = "删除规则";
			$menu2["Syslog Settings"]['plugins/syslog/syslog_reports.php'] = "报警规则";
		}
	}
	$menu = $menu2;
}
...
function syslog_draw_navigation_text ($nav) {
	...
	$nav["syslog_removal.php:"]        = array("title" => "Syslog删除", "mapping" => "index.php:", "url" => $config['url_path'] . "plugins/syslog/syslog_removal.php", "level" => "1");
	$nav["syslog_removal.php:edit"]    = array("title" => "(编辑)", "mapping" => "index.php:,syslog_removal.php:", "url" => "syslog_removal.php", "level" => "2");
	$nav["syslog_removal.php:newedit"] = array("title" => "(编辑)", "mapping" => "index.php:,syslog_removal.php:", "url" => "syslog_removal.php", "level" => "2");
	$nav["syslog_removal.php:actions"] = array("title" => "(操作)", "mapping" => "index.php:,syslog_removal.php:", "url" => "syslog_removal.php", "level" => "2");

	$nav["syslog_alerts.php:"]         = array("title" => "Syslog报警", "mapping" => "index.php:", "url" => $config['url_path'] . "plugins/syslog/syslog_alerts.php", "level" => "1");
	$nav["syslog_alerts.php:edit"]     = array("title" => "(编辑)", "mapping" => "index.php:,syslog_alerts.php:", "url" => "syslog_alerts.php", "level" => "2");
	$nav["syslog_alerts.php:newedit"]  = array("title" => "(编辑)", "mapping" => "index.php:,syslog_alerts.php:", "url" => "syslog_alerts.php", "level" => "2");
	$nav["syslog_alerts.php:actions"]  = array("title" => "(操作)", "mapping" => "index.php:,syslog_alerts.php:", "url" => "syslog_alerts.php", "level" => "2");

	$nav["syslog_reports.php:"]        = array("title" => "Syslog报告", "mapping" => "index.php:", "url" => $config['url_path'] . "plugins/syslog/syslog_reports.php", "level" => "1");
	$nav["syslog_reports.php:edit"]    = array("title" => "(编辑)", "mapping" => "index.php:,syslog_reports.php:", "url" => "syslog_reports.php", "level" => "2");
	$nav["syslog_reports.php:actions"]  = array("title" => "(操作)", "mapping" => "index.php:,syslog_reports.php:", "url" => "syslog_reports.php", "level" => "2");
	...
	if (isset($graph_elements[1]["local_graph_id"])) {
		...
		if (isset($graph_local["host_id"])) {
			...
			if (sizeof($host)) {
				...
				if (sizeof($host)) {
					print "<a href='" . $config["url_path"] . "plugins/syslog/syslog.php?tab=syslog&host%5B%5D=" . $host["host_id"] . "&date1=" . $date1 . "&date2=" . $date2 . "&efacility=0&elevel=0'><img src='" . $config['url_path'] . "plugins/syslog/images/view_syslog.gif' border='0' alt='Display Syslog in Range' title='显示Syslog' style='padding: 3px;'></a><br>";
				}
			}
		}
	}
