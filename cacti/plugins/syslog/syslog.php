...
if (!syslog_check_dependencies()) {
	...
	cacti_log("SYSLOG: 您禁用或卸载了Settings插件，请启用 '<a href='../../plugins.php'>Settings'</a> 插件", true, "SYSTEM");
	print "<br><br><center><font color=red>您禁用或卸载了Settings插件，请启用 '<a href='../../plugins.php'>Settings'</a> 插件</font></color>";
	exit;
}
...
//		"alerts" => "报警日志"
	...
	foreach (array_keys($tabs_syslog) as $tab_short_name) {
		print "<td style='padding:3px 10px 2px 5px;background-color:" . (($tab_short_name == $current_tab) ? "silver;" : "#DFDFDF;") .
			...
			<span class='textHeader'><a href='syslog.php?" ."tab=" . $tab_short_name ."'>$tabs_syslog[$tab_short_name]</a></span>
		</td>\n
		<td width='1'></td>\n";
	}
	}
...
function syslog_view_alarm() {
	...
	echo "<tr><td class='textHeaderDark' style='background-color:#" . $colors["header"] . ";'><strong>查看Syslog报警</strong></td></tr>";
...
function syslog_statistics() {
	...
	html_start_box("<strong>Syslog统计过滤器</strong>", "100%", $colors["header"], "3", "center", "");
	syslog_stats_filter();
	html_end_box();
						...
						<table width='100%' cellspacing='0' cellpadding='0' border='0'>
							<tr>
								<td align='left' class='textHeaderDark'>
									<strong>&lt;&lt; "; if ($_REQUEST["page"] > 1) { $nav .= "<a class='linkOverDark' href='syslog.php?tab=stats&page=" . ($_REQUEST["page"]-1) . "'>"; } $nav .= "上一页"; if ($_REQUEST["page"] > 1) { $nav .= "</a>"; } $nav .= "</strong>
								</td>\n
								...
								<td align='right' class='textHeaderDark'>
									<strong>"; if (($_REQUEST["page"] * $row_limit) < $total_rows) { $nav .= "<a class='linkOverDark' href='syslog.php?tab=stats&page=" . ($_REQUEST["page"]+1) . "'>"; } $nav .= "下一页"; if (($_REQUEST["page"] * $row_limit) < $total_rows) { $nav .= "</a>"; } $nav .= " &gt;&gt;</strong>
								</td>\n
							</tr>
						</table>
					</td>
	...
	$display_text = array(
		"host" => array("主机名", "ASC"),
		"facility" => array("设备", "ASC"),
		"priority" => array("级别", "ASC"),
		"records" => array("事件数量", "DESC"));
	...
	if (sizeof($records)) {
		...
		print "<tr><td colspan='4'><em>未找到Syslog统计</em></td></tr>";
	}
			...
			<table cellpadding="1" cellspacing="0">
				<tr>
					...
					<td width="1">
						...
						<option value="-1"<?php if ($_REQUEST["facility"] == "-1") {?> selected<?php }?>>全部</option>
						<option value="-2"<?php if ($_REQUEST["facility"] == "-2") {?> selected<?php }?>>无</option>
						<?php
					...
					<td width="1">
						...
						<option value="-1"<?php if ($_REQUEST["priority"] == "-1") {?> selected<?php }?>>全部</option>
						<option value="-2"<?php if ($_REQUEST["priority"] == "-2") {?> selected<?php }?>>无</option>
						<?php
					...
					<td>
						&nbsp;<input type="submit" name="clear" value="复位">
					</td>
				</tr>
			</table>
			<table cellpadding="1" cellspacing="0">
...
function get_syslog_messages(&$sql_where, $row_limit, $tab) {
	...
	$data=syslog_db_fetch_assoc($query_sql);
	foreach($data as &$val){
		$val['message']=iconv('GB2312','UTF8//IGNORE',$val['message']);
	}
	return $data;
}
...
function syslog_filter($sql_where, $tab) {
	...
	if (isset($_SESSION["sess_current_date1"])) {
		$filter_text = "</strong> [ 开始: '" . $_SESSION["sess_current_date1"] . "' 结束: '" . $_SESSION["sess_current_date2"] . "' ]";
	}else{
		$filter_text = "</strong>";
	}
	...
	<table width="100%" cellspacing="0" cellpadding="0" border="0">
			...
			<td colspan="2" style="background-color:#EFEFEF;">
				<table width='100%' cellpadding="0" cellspacing="0" border="0">
					<tr>
						<td width='100%'>
							<?php
							html_start_box("<strong>Syslog消息过滤器 $filter_text", "100%", $colors["header"], "1", "center", "");?>
							<tr bgcolor="<?php print $colors["panel"];?>" class="noprint">
									...
									<table cellpadding="0" cellspacing="0" border="0">
										<tr>
											...
											<td width='150' nowrap style='white-space: nowrap;'>
												<input type='text' name='date1' id='date1' title='日志开始时间' size='14' value='<?php print (isset($_SESSION["sess_current_date1"]) ? $_SESSION["sess_current_date1"] : "");?>'>
												&nbsp;<input style='padding-bottom: 4px;' type='image' src='<?php print $config["url_path"];?>images/calendar.gif' alt='Start date selector' title='开始时间选择器' border='0' align='absmiddle' onclick="return showCalendar('date1');">&nbsp;
											</td>
											<td nowrap style='white-space: nowrap;' width='20'>
												&nbsp;<strong>到:</strong>&nbsp;
											</td>
											<td width='150' nowrap style='white-space: nowrap;'>
												<input type='text' name='date2' id='date2' title='日志结束时间' size='14' value='<?php print (isset($_SESSION["sess_current_date2"]) ? $_SESSION["sess_current_date2"] : "");?>'>
												&nbsp;<input style='padding-bottom: 4px;' type='image' src='<?php print $config["url_path"];?>images/calendar.gif' alt='End date selector' title='结束时间选择器' border='0' align='absmiddle' onclick="return showCalendar('date2');">
											</td>
											<td width='125' nowrap style='white-space: nowrap;'>
												&nbsp;&nbsp;<input style='padding-bottom: 4px;' type='image' name='move_left' src='<?php print $config["url_path"];?>images/move_left.gif' alt='Left' border='0' align='absmiddle' title='左移'>
												<select name='predefined_timeshift' title='定义时移周期' onChange="applyTimespanFilterChange(document.syslog_form)">
													<?php
													$start_val = 1;
												...
												<input style='padding-bottom: 4px;' type='image' name='move_right' src='<?php print $config["url_path"];?>images/move_right.gif' alt='Right' border='0' align='absmiddle' title='右移'>
											</td>
											<td>
												&nbsp;<input type="submit" value='确定' name='go' title="确定">
											</td>
											<td>
												&nbsp;<input type='submit' value='清除' name='button_clear_x' title='返回默认时间段'>
											</td>
											<td>
												&nbsp;<input type='submit' value='导出' name='export' title='导出CSV'>
											</td>
											<td>
								...
								<td align='right' style='white-space:nowrap;'>
									<input type='button' value='报警' title='查看Syslog报警规则' onClick='javascript:document.location="/cacti/plugins/syslog/syslog_alerts.php";'>
									<input type='button' value='删除' title='查看Syslog删除规则' onClick='javascript:document.location="/cacti/plugins/syslog/syslog_removal.php";'>
									<input type='button' value='报告' title='查看Syslog报告' onClick='javascript:document.location="/cacti/plugins/syslog/syslog_reports.php";'>&nbsp;
								</td><?php }?>
							</tr>
						</table>
						<table width="100%" cellpadding="0" cellspacing="0" border="0">
								...
								<td>
									<table cellpadding="0" cellspacing="0">
										<tr>
											...
											<td style='padding-right:2px;'>
												<select name="efacility" onChange="javascript:document.getElementById('syslog_form').submit();" title="设备">
													<option value="0"<?php if ($_REQUEST["efacility"] == "0") {?> selected<?php }?>>所有设备</option>
													<?php
											...
											<td style='padding-right:2px;'>
												<select name="elevel" onChange="javascript:document.getElementById('syslog_form').submit();" title="严重级别">
													<option value="0"<?php if ($_REQUEST["elevel"] == "0") {?> selected<?php }?>>所有级别</option>
													<option value="1"<?php if ($_REQUEST["elevel"] == "1") {?> selected<?php }?>>紧急</option>
													<option value="2"<?php if ($_REQUEST["elevel"] == "2") {?> selected<?php }?>>严重++</option>
													<option value="2o"<?php if ($_REQUEST["elevel"] == "2o") {?> selected<?php }?>>严重</option>
													<option value="3"<?php if ($_REQUEST["elevel"] == "3") {?> selected<?php }?>>报警++</option>
													<option value="3o"<?php if ($_REQUEST["elevel"] == "3o") {?> selected<?php }?>>报警</option>
													<option value="4"<?php if ($_REQUEST["elevel"] == "4") {?> selected<?php }?>>错误++</option>
													<option value="4o"<?php if ($_REQUEST["elevel"] == "4o") {?> selected<?php }?>>错误</option>
													<option value="5"<?php if ($_REQUEST["elevel"] == "5") {?> selected<?php }?>>警告++</option>
													<option value="5o"<?php if ($_REQUEST["elevel"] == "5o") {?> selected<?php }?>>警告</option>
													<option value="6"<?php if ($_REQUEST["elevel"] == "6") {?> selected<?php }?>>通知++</option>
													<option value="6o"<?php if ($_REQUEST["elevel"] == "6o") {?> selected<?php }?>>通知</option>
													<option value="7"<?php if ($_REQUEST["elevel"] == "7") {?> selected<?php }?>>信息++</option>
													<option value="7o"<?php if ($_REQUEST["elevel"] == "7o") {?> selected<?php }?>>信息</option>
													<option value="8"<?php if ($_REQUEST["elevel"] == "8") {?> selected<?php }?>>调试</option>
												</select>
											</td>
											<?php if ($_REQUEST["tab"] == "syslog") {?>
											<td style='padding-right:2px;'>
												<select name="removal" onChange="javascript:document.getElementById('syslog_form').submit();" title="删除控制">
													<option value="1"<?php if ($_REQUEST["removal"] == "1") {?> selected<?php }?>>所有记录</option>
													<option value="-1"<?php if ($_REQUEST["removal"] == "-1") {?> selected<?php }?>>主要记录</option>
													<option value="2"<?php if ($_REQUEST["removal"] == "2") {?> selected<?php }?>>删除记录</option>
												</select>
											</td>
											<?php }?>
											<td style='padding-right:2px;'>
												<select name="rows" onChange="javascript:document.getElementById('syslog_form').submit();" title="显示行数">
											...
											<td style='padding-right:2px;'>
												<select name="trimval" onChange="javascript:document.getElementById('syslog_form').submit();" title="文本缩减">
													<option value="1024"<?php if ($_REQUEST["trimval"] == "1024") {?> selected<?php }?>>所有文本</option>
													<option value="30"<?php if ($_REQUEST["trimval"] == "30") {?> selected<?php }?>>30个字符</option>
													<option value="50"<?php if ($_REQUEST["trimval"] == "50") {?> selected<?php }?>>50个字符</option>
													<option value="75"<?php if ($_REQUEST["trimval"] == "75") {?> selected<?php }?>>75个字符</option>
													<option value="100"<?php if ($_REQUEST["trimval"] == "100") {?> selected<?php }?>>100个字符</option>
													<option value="150"<?php if ($_REQUEST["trimval"] == "150") {?> selected<?php }?>>150个字符</option>
													<option value="300"<?php if ($_REQUEST["trimval"] == "300") {?> selected<?php }?>>300个字符</option>
												</select>
											</td>
											<td width="1">
		...
		<tr>
			<td valign="top" style="border-right: #aaaaaa 1px solid;width:1%;padding:10px;" bgcolor='#efefef'>
				<table align="center" cellpadding="1" cellspacing="0" border="0">
					<tr>
						<td>
							...
							<tr>
								<td>
									<select title="主机过滤器" id="host_select" name="host[]" multiple size="20" style="width: 150px; overflow: scroll; height: auto;" onChange="javascript:document.getElementById('syslog_form').submit();">
										<?php if ($tab == "syslog") { ?><option id="host_all" value="0"<?php if (((is_array($_REQUEST["host"])) && ($_REQUEST["host"][0] == "0")) || ($reset_multi)) {?> selected<?php }?>>显示所有主机</option><?php }else{?>
										<option id="host_all" value="0"<?php if (((is_array($_REQUEST["host"])) && ($_REQUEST["host"][0] == "0")) || ($reset_multi)) {?> selected<?php }?>>显示所有日志</option>
										<option id="host_none" value="-1"<?php if (((is_array($_REQUEST["host"])) && ($_REQUEST["host"][0] == "-1"))) {?> selected<?php }?>>阈值日志</option><?php }?>
										<?php
										$hosts_where = "";
...
function syslog_syslog_legend() {
	...
	print "<td width='10%' style='text-align:center;color:#$fg_color;background-color:#$bg_color;'><b>紧急</b></td>";
	...
	print "<td width='10%' style='text-align:center;color:#$fg_color;background-color:#$bg_color;'><b>严重</b></td>";
	...
	print "<td width='10%' style='text-align:center;color:#$fg_color;background-color:#$bg_color;'><b>报警</b></td>";
	...
	print "<td width='10%' style='text-align:center;color:#$fg_color;background-color:#$bg_color;'><b>错误</b></td>";
	...
	print "<td width='10%' style='text-align:center;color:#$fg_color;background-color:#$bg_color;'><b>警告</b></td>";
	...
	print "<td width='10%' style='text-align:center;color:#$fg_color;background-color:#$bg_color;'><b>通知</b></td>";
	...
	print "<td width='10%' style='text-align:center;color:#$fg_color;background-color:#$bg_color;'><b>信息</b></td>";
	...
	print "<td width='10%' style='text-align:center;color:#$fg_color;background-color:#$bg_color;'><b>调试</b></td>";
...
function syslog_messages($tab="syslog") {
		...
		$nav = "<tr bgcolor='#" . $colors["header"] . "'>
					<td colspan='13'>
						<table width='100%' cellspacing='0' cellpadding='0' border='0'>
							<tr>
								<td align='left' class='textHeaderDark'>
									<strong>&lt;&lt; "; if ($_REQUEST["page"] > 1) { $nav .= "<a class='linkOverDark' href='syslog.php?tab=$tab&page=" . ($_REQUEST["page"]-1) . "'>"; } $nav .= "上一页"; if ($_REQUEST["page"] > 1) { $nav .= "</a>"; } $nav .= "</strong>
								</td>\n
								<td align='center' class='textHeaderDark'>
									" . ($total_rows == 0 ? "无" : (($row_limit*($_REQUEST["page"]-1))+1) . " 到 " . ((($total_rows < $row_limit) || ($total_rows < ($row_limit*$_REQUEST["page"]))) ? $total_rows : ($row_limit*$_REQUEST["page"])) . " 行,共 $total_rows 行,第 [$url_page_select]") . " 页
								</td>\n
								<td align='right' class='textHeaderDark'>
									<strong>"; if (($_REQUEST["page"] * $row_limit) < $total_rows) { $nav .= "<a class='linkOverDark' href='syslog.php?tab=$tab&page=" . ($_REQUEST["page"]+1) . "'>"; } $nav .= "下一页"; if (($_REQUEST["page"] * $row_limit) < $total_rows) { $nav .= "</a>"; } $nav .= " &gt;&gt;</strong>
								</td>\n
							</tr>
						</table>
					</td>
	...
	if ($tab == "syslog") {
		if (api_plugin_user_realm_auth('syslog_alerts.php')) {
			$display_text = array(
				"nosortt" => array("动作", "ASC"),
				"host_id" => array("主机", "ASC"),
				"logtime" => array("时间", "ASC"),
				"message" => array("消息", "ASC"),
				"facility_id" => array("设备", "ASC"),
				"priority_id" => array("级别", "ASC"));
		}else{
			$display_text = array(
				"host_id" => array("主机", "ASC"),
				"logtime" => array("时间", "ASC"),
		...
		if (sizeof($syslog_messages) > 0) {
			foreach ($syslog_messages as $syslog_message) {
				...
				if (api_plugin_user_realm_auth('syslog_alerts.php')) {
					print "<td style='whitspace-nowrap;width:40px;'>";
		...
		if (sizeof($syslog_messages) > 0) {
			foreach ($syslog_messages as $log) {
				...
				print "<td>" . (strlen($log["name"]) ? $log["name"]:"报警已被删除") . "</td>\n";
				print "<td>" . (isset($severities[$log["severity"]]) ? $severities[$log["severity"]]:"未知") . "</td>\n";
