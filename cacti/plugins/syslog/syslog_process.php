...
if (sizeof($query)) {
	foreach($query as $alert) {
		...
		if ($sql != '') {
			...
			if (($alert['method'] == "1" && $count >= $alert["num"]) || ($alert["method"] == "0")) {
				...
				if (sizeof($at)) {
					...
					if ($alert['method'] == "1") {
						$alertm .= "-----------------------------------------------\n";
						$alertm .= "警告	: Syslog实例记数器报警已被触发". "\n";
						$alertm .= "名称	: " . $alert['name']     . "\n";
						$alertm .= "严重级别	: " . $severities[$alert['severity']] . "\n";
						$alertm .= "阈值	: " . $alert['num'] . "\n";
						$alertm .= "记数器	: " . sizeof($at)       . "\n";
						$alertm .= "消息	: " . $alert['message'] . "\n";

						$htmlm  .= "<body class='body'><h1 class='h1'>Syslog阈值报警 '" . $alert['name'] . "'</h1>";
						$htmlm  .= "<table class='table' cellspacing='0' cellpadding='3' border='1'>";
						$htmlm  .= "<tr><th class='th'>报警名称</th><th class='th'>严重级别</th><th class='th'>阈值</th><th class='th'>记数器</th><th class='th'>匹配字符串</th></tr>";
						...
						$htmlm .= "<body class='body'><h1 class='h1'>Syslog报警 '" . $alert['name'] . "'</h1>";
					}

					$htmlm .= "<table  class='table' cellspacing='0' cellpadding='3' border='1'>";
					$htmlm .= "<tr><th class='th'>主机名</th><th class='th'>时间</th><th class='th'>严重级别</th><th class='th'>设备</th><th class='th'>关键字</th><th class='th'>消息</th></tr>";
					...
					foreach($at as $a) {
						...
						if (($alert["method"] == 1 && $alert_count < $max_alerts) || $alert["method"] == 0) {
							...
							$alertm .= "-----------------------------------------------\n";
							$alertm .= '主机名	: ' . $a['host'] . "\n";
							$alertm .= '时间	: ' . $a['date'] . ' ' . $a['time'] . "\n";
							$alertm .= '严重级别	: ' . $severities[$alert['severity']] . "\n";
							$alertm .= '设备	: ' . $a['priority'] . "\n";
							$alertm .= '关键字	: ' . $alert['message'] . "\n";
							$alertm .= '消息	:'  . "\n" . $a['message'] . "\n";
							...
							$htmlm  .= "<td class='td'>"     . $alert['message']               . "</td>"      . "\n";
		...
		if ($alertm != '') {
			...
			if ($resend) {
				syslog_sendemail(trim($alert['email']), '', '事件报警 - ' . $alert['name'], ($html ? $htmlm:$alertm), $smsalert);
				if ($alert['method'] != "1") {
