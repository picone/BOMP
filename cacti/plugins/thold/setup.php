...
function thold_graph_button ($data) {
	...
	if (api_user_realm_auth('thold_graph.php')) {
		print '<a href="' .  $url . $separator . 'thold_vrule=' . ($_SESSION['sess_config_array']['thold_draw_vrules'] == 'on' ? 'off' : 'on') . '"><img src="' . $config['url_path'] . 'plugins/thold/images/reddot.png" border="0" alt="Thresholds" title="' . ($_SESSION['sess_config_array']['thold_draw_vrules'] == 'on' ? '关闭阈值' : '打开阈值') . '" style="padding: 3px;"></a><br>';
	}
...
function thold_device_action_array($device_action_array) {
	$device_action_array['thold'] = '应用阈值';
...
function thold_data_sources_table ($ds) {
	...
	if (!isset($ds['data_source'])) {
		$ds['data_template_name'] = ((empty($ds['data_template_name'])) ? '<em>无</em>' : $ds['data_template_name']);
	} else {
		$ds['template_name'] = ((empty($ds['data_source']['data_template_name'])) ? '<em>无</em>' : $ds['data_source']['data_template_name']);
	}
	return $ds;
}
...
function thold_graphs_new () {
	...
	print '<span style="color: #c16921;">*</span><a href="' . $config['url_path'] . 'plugins/thold/thold.php?action=autocreate&hostid=' . $_REQUEST['host_id'] . '">自动添加阈值</a><br>';
}
...
function thold_data_source_action_prepare($save) {
	...
	if ($save["drp_action"] == "plugin_thold_create") {
		...
		if (strlen($found_list)) {
			if (strlen($not_found)) {
				print "<p>没有阈值模板关联到以下数据源</p>";
				print "<p>" . $not_found . "</p>";
			}

			print "<p>您真的希望为这些数据源添加阈值吗?
					<p>" . $found_list . "</p>
					</td>
				</tr>\n
				";
...
function thold_data_source_action_array($action) {
	$action['plugin_thold_create'] = '从模板添加阈值';
	return $action;
}
					...
					foreach ($rrdlist as $rrdrow) {
						...
						if (count($existing) == 0) {
							...
							if ($id) {
								...
								plugin_thold_log_changes($id, 'created', " $tname [$name]");
								$message .= "从图形: '<i>$tname</i>' 添加阈值,使用数据源: '<i>$name</i>'<br>";
							}
						}
					}
				}
