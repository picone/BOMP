				...
				<tr>
					...
					<td width="1">
						<select name="data_template_id" onChange="applyTHoldFilterChange(document.form_thold)">
							<option value="-1"<?php if ($_REQUEST["data_template_id"] == "-1") {?> selected<?php }?>>全部</option>
							<option value="0"<?php if ($_REQUEST["data_template_id"] == "0") {?> selected<?php }?>>无</option>
							<?php
					...
					<td width="1">
						<select name="triggered" onChange="applyTHoldFilterChange(document.form_thold)">
							<option value="-1"<?php if ($_REQUEST["triggered"] == "-1") {?> selected<?php }?>>全部</option>
							<option value="1"<?php if ($_REQUEST["triggered"] == "1") {?> selected<?php }?>>已触发</option>
							<option value="2"<?php if ($_REQUEST["triggered"] == "2") {?> selected<?php }?>>已启用</option>
							<option value="0"<?php if ($_REQUEST["triggered"] == "0") {?> selected<?php }?>>已禁用</option>
						</select>
					</td>
					<td nowrap style='white-space:nowrap;' width="50">
						&nbsp;行:&nbsp;
...
function tholds() {
	...
	html_start_box("<strong>阈值状态</strong>", "100%", $colors["header"], "3", "center", "");
	form_thold_filter();
	html_end_box();
				...
				<table width='100%' cellspacing='0' cellpadding='0' border='0'>
					<tr>
						<td align='left' class='textHeaderDark'>
							<strong>&lt;&lt; "; if ($_REQUEST["page"] > 1) { $nav .= "<a class='linkOverDark' href='thold_graph.php?filter=" . $_REQUEST["filter"] . "&page=" . ($_REQUEST["page"]-1) . "'>"; } $nav .= "上一页"; if ($_REQUEST["page"] > 1) { $nav .= "</a>"; } $nav .= "</strong>
						</td>\n
						<td align='center' class='textHeaderDark'>
							" . (($_REQUEST["rows"]*($_REQUEST["page"]-1))+1) . " 到 " . ((($total_rows < read_config_option("num_rows_device")) || ($total_rows < ($_REQUEST["rows"]*$_REQUEST["page"]))) ? $total_rows : ($_REQUEST["rows"]*$_REQUEST["page"])) . " 行,共 $total_rows 行 [ 第 $url_page_select 页 ]
						</td>\n
						<td align='right' class='textHeaderDark'>
							<strong>"; if (($_REQUEST["page"] * $_REQUEST["rows"]) < $total_rows) { $nav .= "<a class='linkOverDark' href='thold_graph.php?filter=" . $_REQUEST["filter"] . "&page=" . ($_REQUEST["page"]+1) . "'>"; } $nav .= "下一页"; if (($_REQUEST["page"] * $_REQUEST["rows"]) < $total_rows) { $nav .= "</a>"; } $nav .= " &gt;&gt;</strong>
						</td>\n
					</tr>
				</table>
			</td>
	...
	if (count($result)) {
		foreach ($result as $row) {
			...
			if (api_user_realm_auth('thold_add.php')) {
				print '<a href="' .  $config['url_path'] . 'plugins/thold/thold.php?rra=' . $row["rra_id"] . '&view_rrd=' . $row["data_id"] .'"><img src="' . $config['url_path'] . 'plugins/thold/images/edit_object.png" border="0" alt="Edit Threshold" title="编辑阈值"></a>';
			}
			if ($row["thold_enabled"] == 'on') {
				print '<a href="' .  $config['url_path'] . 'plugins/thold/thold.php?id=' . $row["id"] .'&action=disable"><img src="' . $config['url_path'] . 'plugins/thold/images/disable_thold.png" border="0" alt="Disable Threshold" title="禁用阈值"></a>';
			}else{
				print '<a href="' .  $config['url_path'] . 'plugins/thold/thold.php?id=' . $row["id"] . '&action=enable"><img src="' . $config['url_path'] . 'plugins/thold/images/enable_thold.png" border="0" alt="Enable Threshold" title="启用阈值"></a>';
			}
			print "<a href='". $config['url_path'] . "graph.php?local_graph_id=" . $row['graph_id'] . "&rra_id=all'><img src='" . $config['url_path'] . "plugins/thold/images/view_graphs.gif' border='0' alt='View Graph' title='查看图形'></a>";

			print "</td>";
			print "<td>" . ($row['name'] != '' ? $row['name'] : 'No name set') . "</td>";
			print "<td width='30'>" . $row["id"] . "</td>";
			...
			if ($row['thold_enabled'] == 'off') {
				print "<td width='40'><b>已禁用</b></td>";
			}else{
				print "<td width='40'>已启用</td>";
			}
			form_end_row();
		}
	} else {
...
function hosts() {
	...
	html_start_box("<strong>设备状态</strong>", "100%", $colors["header"], "3", "center", "");
	form_host_filter();
	html_end_box();
	...
	$nav = "<tr bgcolor='#" . $colors["header"] . "'>
			<td colspan='11'>
				<table width='100%' cellspacing='0' cellpadding='0' border='0'>
					<tr>
						<td align='left' class='textHeaderDark'>
							<strong>&lt;&lt; "; if ($_REQUEST["page"] > 1) { $nav .= "<a class='linkOverDark' href='thold_graph.php?filter=" . $_REQUEST["filter"] . "&host_template_id=" . $_REQUEST["host_template_id"] . "&host_status=" . $_REQUEST["host_status"] . "&page=" . ($_REQUEST["page"]-1) . "'>"; } $nav .= "上一页"; if ($_REQUEST["page"] > 1) { $nav .= "</a>"; } $nav .= "</strong>
						</td>\n
						<td align='center' class='textHeaderDark'>
							" . (($_REQUEST["rows"]*($_REQUEST["page"]-1))+1) . " 到 " . ((($total_rows < read_config_option("num_rows_device")) || ($total_rows < ($_REQUEST["rows"]*$_REQUEST["page"]))) ? $total_rows : ($_REQUEST["rows"]*$_REQUEST["page"])) . " 行,共 $total_rows 行 [ 第 $url_page_select 页 ]
						</td>\n
						<td align='right' class='textHeaderDark'>
							<strong>"; if (($_REQUEST["page"] * $_REQUEST["rows"]) < $total_rows) { $nav .= "<a class='linkOverDark' href='thold_graph.php?filter=" . $_REQUEST["filter"] . "&host_template_id=" . $_REQUEST["host_template_id"] . "&host_status=" . $_REQUEST["host_status"] . "&page=" . ($_REQUEST["page"]+1) . "'>"; } $nav .= "下一页"; if (($_REQUEST["page"] * $_REQUEST["rows"]) < $total_rows) { $nav .= "</a>"; } $nav .= " &gt;&gt;</strong>
						</td>\n
					</tr>
				</table>
			</td>
	...
	$display_text = array(
		...
		"id" => array("编号", "ASC"),
		"nosort1" => array("图形", "ASC"),
		"nosort2" => array("数据源", "ASC"),
		"status" => array("状态", "ASC"),
		"status_event_count" => array("事件数量", "ASC"),
		"hostname" => array("主机名", "ASC"),
		"cur_time" => array("当前(毫秒)", "DESC"),
		"avg_time" => array("平均(毫秒)", "DESC"),
		"availability" => array("可用性", "ASC"));
		...
		foreach ($hosts as $host) {
			...
			if ($host["availability_method"] != 0) {
				...
				if (api_user_realm_auth('host.php')) {
					print '<a href="' .  $config['url_path'] . 'host.php?action=edit&id=' . $host["id"] . '"><img src="' . $config['url_path'] . 'plugins/thold/images/edit_object.png" border="0" alt="Edit Host" title="编辑主机"></a>';
				}
				print "<a href='". $config['url_path'] . "graph_view.php?action=preview&graph_template_id=0&filter=&host_id=" . $host["id"] . "'><img src='" . $config['url_path'] . "plugins/thold/images/view_graphs.gif' border='0' alt='View Graphs' title='查看图形'></a>";
				print "</td>";
				?>
				<td>
				...
				<td width='80'><?php print round(($host["cur_time"]), 2);?></td>
				<td width='80'><?php print round(($host["avg_time"]), 2);?></td>
				...
				print "<a href='". $config['url_path'] . "graph_view.php?action=preview&graph_template_id=0&filter=&host_id=" . $host["id"] . "'><img src='" . $config['url_path'] . "plugins/thold/images/view_graphs.gif' border='0' alt='View Graphs' title='查看图形View Graphs'></a>";
				print "</td>";
				?>
				<td>
					<?php print (strlen($_REQUEST["filter"]) ? eregi_replace("(" . preg_quote($_REQUEST["filter"]) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", $host["description"]) : $host["description"]);?>
				...
				<tr>
					...
					<td width="1">
						<select name="host_template_id" onChange="applyViewDeviceFilterChange(document.form_devices)">
							<option value="-1"<?php if ($_REQUEST["host_template_id"] == "-1") {?> selected<?php }?>>全部</option>
							<option value="0"<?php if ($_REQUEST["host_template_id"] == "0") {?> selected<?php }?>>无</option>
							<?php
					...
					<td width="1">
						<select name="host_status" onChange="applyViewDeviceFilterChange(document.form_devices)">
							<option value="-1"<?php if ($_REQUEST["host_status"] == "-1") {?> selected<?php }?>>全部</option>
							<option value="-3"<?php if ($_REQUEST["host_status"] == "-3") {?> selected<?php }?>>已启用</option>
							<option value="-2"<?php if ($_REQUEST["host_status"] == "-2") {?> selected<?php }?>>已禁用</option>
							<option value="-4"<?php if ($_REQUEST["host_status"] == "-4") {?> selected<?php }?>>不在线</option>
							<option value="-5"<?php if ($_REQUEST["host_status"] == "-5") {?> selected<?php }?>>未监视</option>
							<option value="3"<?php if ($_REQUEST["host_status"] == "3") {?> selected<?php }?>>在线</option>
							<option value="1"<?php if ($_REQUEST["host_status"] == "1") {?> selected<?php }?>>宕机</option>
							<option value="2"<?php if ($_REQUEST["host_status"] == "2") {?> selected<?php }?>>恢复中</option>
							<option value="0"<?php if ($_REQUEST["host_status"] == "0") {?> selected<?php }?>>未知</option>
						</select>
					</td>
					<td nowrap style='white-space:nowrap;' width='1'>
						&nbsp;行:&nbsp;
