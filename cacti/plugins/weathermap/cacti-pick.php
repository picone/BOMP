...
<?php 
if(sizeof($hosts) > 0) {
	print '任意: <select name="host_id"  onChange="applyDSFilterChange(document.mini)">';

	print '<option '.($host_id==-1 ? 'SELECTED' : '' ).' value="-1">任意</option>';
	print '<option '.($host_id==0 ? 'SELECTED' : '' ).' value="0">无</option>';
	...
	print '<span class="filter" style="display: none;">过滤器: <input id="filterstring" name="filterstring" size="20"> (大小写敏感)<br /></span>';
...
<?php 
if(sizeof($hosts) > 0) {
	print '主机: <select name="host_id"  onChange="applyDSFilterChange(document.mini)">';
	...
	print '<span class="filter" style="display: none;">过滤器: <input id="filterstring" name="filterstring" size="20"> (大小写敏感)<br /></span>';
	print '<input id="overlib" name="overlib" type="checkbox" value="yes" '.($overlib ? 'CHECKED' : '' ).'> <label for="overlib">同时设置悬停图形和信息URL.</label><br />';
	// print '<input id="aggregate" name="aggregate" type="checkbox" value="yes" '.($aggregate ? 'CHECKED' : '' ).'> <label for="aggregate">Append TARGET to existing one (Aggregate)</label>';
