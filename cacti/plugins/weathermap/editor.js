...
var helptexts = {
    "link_target": 'Cacti应当从哪里获取这个连接的数据?可以是RRD文件或HTML.',
    "link_width": '连接箭头的宽度,单位:像素.',
    "link_infourl":
        '如果您正在使用 \'overlib\' HTML样式,当您点击连接时将打开的URL',
    "link_hover":
        '如果您正在使用 \'overlib\' HTML样式,当您悬停在连接上显示的图形',
    "link_bandwidth_in": "第一个节点流向第二个节点的带宽",
    "link_bandwidth_out": "第二个节点流向第一个节点的带宽(如果上下行不相同)",
    "link_commentin": "连接旁边显示的文字",
    "link_commentout": "连接旁边显示的文字",

    "node_infourl":
        '如果您正在使用 \'overlib\' HTML样式,当您点击节点时将打开的URL',
    "node_hover":
        '如果您正在使用 \'overlib\' HTML样式,当您悬停在节点上显示的图形',
    "node_x": "这个点离左边有多少像素",
    "node_y": "这个点离顶部有多少像素",
    "node_label": "节点的标签",
    "node_new_name": "这个节点的名称",

    "tb_newfile": '切换到另一个文件或重新建立一个.',
    "tb_addnode": '添加一个新节点',
    "tb_addlink": '在两个节点之间添加一个新连接.',


    "hover_tb_newfile": '点击编辑另一个气象图或建立一个新气象图.',
...
function coord_update(event)
{
    ...
    jQuery('#tb_coords').html('位置<br />'+ cursorx + ', ' + cursory);
}

function coord_release(event)
{
    jQuery('#tb_coords').html('位置<br />---, ---');
}
