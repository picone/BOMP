	...
	<div align="center" id="mainarea">
	 ...
	 <input style="display:none" type="image"
	  ...
	  "<?php echo  $imageurl; ?>" id="existingdata" alt="Weathermap" usemap="#weathermap_imap"
	   />
		<div class="debug" style="display:none;"><p><strong>调试:</strong> <a href="?<?php echo ($fromplug==TRUE ? 'plug=1&amp;' : ''); ?>action=nothing&amp;mapname=<?php echo  $mapname ?>">什么都不做</a>
	   <span><label for="mapname">地图文件</label><input type="text" name="mapname" value="<?php echo  $mapname; ?>" /></span>
	   <span><label for="action">操作</label><input type="text" id="action" name="action" value="<?php echo $newaction; ?>" /></span>
	  <span><label for="param">参数</label><input type="text" name="param" id="param" value="" /></span>
            <span><label for="param2">参数2</label><input type="text" name="param2" id="param2" value="<?php echo $param2; ?>" /></span>
	  <span><label for="debug">调试</label><input id="debug" value="" name="debug" /></span>
	  <a target="configwindow" href="?<?php echo ($fromplug==TRUE ? 'plug=1&amp;':''); ?>action=show_config&amp;mapname=<?php echo  $mapname ?>">查看配置文件</a></p>
...
?>
	...
	<div id="dlgNodeProperties" class="dlgProperties">
	  <div class="dlgTitlebar">
		...
		<ul>
		  <li><a id="tb_node_submit" class="wm_submit" title="提交所有更改">提交</a></li>
		  <li><a id="tb_node_cancel" class="wm_cancel" title="取消所有更改">取消</a></li>
		</ul>
	  </div>
	  ...
	  <div class="dlgBody">
		...
		<tr>
		  ...
		  <tr>
			...
			<span class="cactinode"><a id="node_cactipick">[从Cacti选择]</a></span></td>
		  </tr>
		  <tr>
			<th>图标文件</th>
...
<?php
	if(count($imlist)==0)
	{
		print '<option value="--无--">(无可用图片)</option>';
	}
	else
	{
		print '<option value="--NONE--">--无图标--</option>';
		print '<option value="--AICON--">--人造图标--</option>';
...
?>
		  ...
		  <tr>
			<th></th>
			<td><a id="node_move" class="dlgTitlebar">移动</a><a class="dlgTitlebar" id="node_delete">删除</a><a class="dlgTitlebar" id="node_clone">克隆</a><a class="dlgTitlebar" id="node_edit">编辑</a></td>
		  </tr>
		</table>
	  </div>
	...
	<div id="dlgLinkProperties" class="dlgProperties">
	  <div class="dlgTitlebar">
		...
		<ul>
		  <li><a title="提交所有更改"  class="wm_submit" id="tb_link_submit">提交</a></li>
		  <li><a title="取消所有更改" class="wm_cancel" id="tb_link_cancel">取消</a></li>
		</ul>
	  </div>

	  <div class="dlgBody">
		<div class="comment">
		  连接 '<span id="link_nodename1">%NODE1%</span>' 到 '<span id="link_nodename2">%NODE2%</span>'
		</div>
		...
		<input size="6" name="link_name" type="hidden" />
		  ...
		  <table width="100%">
			<tr>
			  <th>最大带宽<br />
			  流入 '<span id="link_nodename1a">%NODE1%</span>'</th>
			  <td><input size="8" id="link_bandwidth_in" name="link_bandwidth_in" type=
			  "text" /> 位/秒</td>
			</tr>
			<tr>
			  <th>最大带宽<br />
			  流出 '<span id="link_nodename1b">%NODE1%</span>'</th>
			  <td><input type="checkbox" id="link_bandwidth_out_cb" name=
			  "link_bandwidth_out_cb" value="symmetric" />与'流入'相同或
			  <input id="link_bandwidth_out" name="link_bandwidth_out"
			  size="8" type="text" /> 位/秒</td>
			</tr>
			<tr>
			  <th>数据源</th>
			  <td><input id="link_target" name="link_target" type="text" /> <span class="cactilink"><a id="link_cactipick">[从Cacti选择]</a></span></td>
			</tr>
			<tr>
			  <th>连接带宽</th>
			...
			<tr>
			  <th></th>
			  <td><a class="dlgTitlebar" id="link_delete">删除连接</a><a class="dlgTitlebar" id="link_edit">编辑</a><a
                            class="dlgTitlebar" id="link_vert">垂直</a><a
                            class="dlgTitlebar" id="link_horiz">水平</a><a 
							class="dlgTitlebar" id="link_via">经过</a> 
                        </td>
			</tr>
		  </table>
	  </div>
	...
	<div id="dlgMapProperties" class="dlgProperties">
	  <div class="dlgTitlebar">
		...
		<ul>
		  <li><a title="提交所有更改"  class="wm_submit" id="tb_map_submit">提交</a></li>
		  <li><a title="取消所有更改" class="wm_cancel" id="tb_map_cancel">取消</a></li>
		</ul>
	  </div>
	  ...
	  <div class="dlgBody">
		...
		<tr>
			<th>默认连接线宽度</th>
			<td><input name="map_linkdefaultwidth" size="6" type="text" value="<?php echo  $map->links['DEFAULT']->width ?>" /> 像素</td>
		  </tr>

		<tr>
			<th>默认连接带宽</th>
			<td><input name="map_linkdefaultbwin" size="6" type="text" value="<?php echo  $map->links['DEFAULT']->max_bandwidth_in_cfg ?>" /> 流入 位/秒, <input name="map_linkdefaultbwout" size="6" type="text" value="<?php echo  $map->links['DEFAULT']->max_bandwidth_out_cfg ?>" /> 流出 位/秒</td>
		  </tr>
		  ...
		  <tr>
			...
			"text"  value="<?php echo  $map->height ?>" /> 像素</td>
		  </tr>
		   <tr>
			<th>输出图片文件名</th>
...
<?php
	if(count($imlist)==0)
	{
		print '<option value="--NONE--">(无可用图片)</option>';
	}
	else
	{
		print '<option value="--NONE--">--无--</option>';
		foreach ($imlist as $im)
		{
			print "<option ";
...
?>
	...
	<div id="dlgMapStyle" class="dlgProperties">
	  <div class="dlgTitlebar">
		...
		<ul>
		  <li><a title="提交所有更改" id="tb_mapstyle_submit" class="wm_submit" >提交</a></li>
		  <li><a title="取消所有更改" class="wm_cancel" id="tb_mapstyle_cancel">取消</a></li>
		</ul>
	  </div>
		...
		<table>
		  <tr>
			...
			<td><select id="mapstyle_linklabels" name="mapstyle_linklabels">
			  <option <?php echo ($map->links['DEFAULT']->labelstyle=='bits' ? 'selected' : '') ?> value="bits">位/秒</option>
			  <option <?php echo ($map->links['DEFAULT']->labelstyle=='percent' ? 'selected' : '') ?> value="percent">百分比</option>
			  <option <?php echo ($map->links['DEFAULT']->labelstyle=='none' ? 'selected' : '') ?> value="none">无</option>
			</select></td>
		  </tr>
		  <tr>
			<th>HTML样式</th>
			<td><select name="mapstyle_htmlstyle">
			  ...
			  <option <?php echo ($map->htmlstyle=='static' ? 'selected' : '') ?> value="static">静态HTML</option>
			</select></td>
		  </tr>
		  <tr>
			<th>箭头</th>
			<td><select name="mapstyle_arrowstyle">
			  <option <?php echo ($map->links['DEFAULT']->arrowstyle=='classic' ? 'selected' : '') ?> value="classic">经典</option>
			  <option <?php echo ($map->links['DEFAULT']->arrowstyle=='compact' ? 'selected' : '') ?> value="compact">紧密</option>
			</select></td>
		  </tr>
		  <tr>
			<th>节点字体</th>
		  ...
		  <tr>
			<th>字体:</th>
			<td><div class="fontsamples" ><img alt="Sample of defined fonts" src="?action=font_samples&mapname=<?php echo  $mapname?>" /></div><br />(使用PHP绘图)</td>
		  </tr>
		</table>
	  </div>
	...
	<div id="dlgColours" class="dlgProperties">
	  <div class="dlgTitlebar">
		...
		<ul>
		  <li><a title="提交所有更改" id="tb_colours_submit"  class="wm_submit" >提交</a></li>
		  <li><a title="取消所有更改" class="wm_cancel" id="tb_colours_cancel">取消</a></li>
		</ul>
	  </div>
	...
	<div id="dlgImages" class="dlgProperties">
	  <div class="dlgTitlebar">
		...
		<ul>
		  <li><a title="提交所有更改" id="tb_images_submit"  class="wm_submit" >提交</a></li>
		  <li><a title="取消所有更改" class="wm_cancel" id="tb_images_cancel">取消</a></li>
		</ul>
	  </div>
	  ...
	  <div class="dlgTitlebar">
		编辑气象图对象
		<ul>
		  <li><a title="提交所有更改" id="tb_textedit_submit"  class="wm_submit" >提交</a></li>
		  <li><a title="取消所有更改" class="wm_cancel" id="tb_textedit_cancel">取消</a></li>
		</ul>
	  </div>
	...
	<div id="dlgEditorSettings" class="dlgProperties">
	  <div class="dlgTitlebar">
		编辑器设置
		<ul>
		  <li><a title="提交更改" id="tb_editorsettings_submit"  class="wm_submit" >提交</a></li>
		  <li><a title="取消更改" class="wm_cancel" id="tb_editorsettings_cancel">取消</a></li>
		</ul>
	  </div>
	  ...
	  <div class="dlgBody">
		<table>
			...
			<td><select id="editorsettings_showvias" name="editorsettings_showvias">
			  <option <?php echo ($use_overlay ? 'selected' : '') ?> value="1">是</option>
			  <option <?php echo ($use_overlay ? '' : 'selected') ?> value="0">否</option>
			    </select>
			</td>
		    </tr>
		    <tr>
			...
			<td><select id="editorsettings_showrelative" name="editorsettings_showrelative">
			  <option <?php echo ($use_relative_overlay ? 'selected' : '') ?> value="1">是</option>
			  <option <?php echo ($use_relative_overlay ? '' : 'selected') ?> value="0">否</option>
			    </select>
			</td>
		    </tr>
		    <tr>
			...
			<td><select id="editorsettings_gridsnap" name="editorsettings_gridsnap">
			    <option <?php echo ($grid_snap_value==0 ? 'selected' : '') ?> value="NO">否</option>
			    <option <?php echo ($grid_snap_value==5 ? 'selected' : '') ?> value="5">5像素</option>
			    <option <?php echo ($grid_snap_value==10 ? 'selected' : '') ?> value="10">10像素</option>
			    <option <?php echo ($grid_snap_value==15 ? 'selected' : '') ?> value="15">15像素</option>
			    <option <?php echo ($grid_snap_value==20 ? 'selected' : '') ?> value="20">20像素</option>
			    <option <?php echo ($grid_snap_value==50 ? 'selected' : '') ?> value="50">50像素</option>
			    <option <?php echo ($grid_snap_value==100 ? 'selected' : '') ?> value="100">100像素</option>
			    </select>
			</td>
		    </tr>
		</table>
