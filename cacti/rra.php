...
function rra_remove() {
	...
	if ((read_config_option("deletion_verification") == "on") && (!isset($_GET["confirm"]))) {
		include_once("./include/top_header.php");
		form_confirm("您确定吗?", "您真的想要删除循环归档 <strong>'" . htmlspecialchars(db_fetch_cell("select name from rra where id=" . $_GET["id"])) . "'</strong>?", htmlspecialchars("rra.php"), htmlspecialchars("rra.php?action=remove&id=" . $_GET["id"]));
		exit;
	}
...
function rra_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . htmlspecialchars($rra["name"]) . "]";
	}else{
		$header_label = "[新]";
	}

	html_start_box("<strong>循环归档</strong> $header_label", "100%", $colors["header"], "3", "center", "");
...
function rra() {
	...
	html_start_box("<strong>循环归档</strong>", "100%", $colors["header"], "3", "center", "rra.php?action=edit");

	$display_text = array(
		"name" => array("名称", "ASC"),
		"steps" => array("步进", "ASC"),
		"rows" => array("行", "ASC"),
		"timespan" => array("时间段", "ASC"));
