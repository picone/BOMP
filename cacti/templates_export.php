...
function export() {
	...
	html_start_box("<strong>导出模板</strong> [" . $export_types{$_REQUEST["export_type"]}["name"] . "]", "100%", $colors["header"], "3", "center", "");

	form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],0); ?>
		<td width="50%">
			<font class="textEditTitle">导出<?php print $export_types{$_REQUEST["export_type"]}["name"];?></font><br>
			选择对象导出到XML文件.
		</td>
		<td>
	...
	<?php form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],1); ?>
		<td width="50%">
			<font class="textEditTitle">包括依赖关系</font><br>
			某些模板依赖其它对象才能正常运行.强烈建议您选择,否则导出的模板可能在导入后无法正常工作.
		</td>
		<td>
			<?php form_checkbox("include_deps", "on", "包括信赖关系", "on", "", true);?>
		</td>
	</tr>

	<?php form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],0); ?>
		<td width="50%">
			<font class="textEditTitle">输出格式</font><br>
			选择XML的输出方式.
		</td>
		<td>
			<?php
			form_radio_button("output_format", "3", "1", "输出到浏览器(显示在Cacti里)","1",true); print "<br>";
			form_radio_button("output_format", "3", "2", "输出到浏览器(纯XML)","1",true); print "<br>";
			form_radio_button("output_format", "3", "3", "保存到本地","1",true);
