	...
	if (isset($user_array) && sizeof($user_array)) {
			...
			print "
				<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"] . "'>
						<p>当您点击 \"继续\",已选用户将被删除.</p>
						<p><ul>$user_list</ul></p>
					</td>
				</tr>\n";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='删除用户'>";
		}
		$user_id = "";
		...
		if ((get_request_var_post("drp_action") == "2") && (sizeof($user_array))) { /* copy */
			...
			print "
				<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"] . "'>
						当您点击 \"继续\" 已选用户将被复制到新用户<br><br>
					</td>
				</tr><tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"] . "'>
						模板用户名: <i>" . db_fetch_cell("SELECT username FROM user_auth WHERE id=" . $user_id) . "</i>
					</td>
				</tr><tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"] . "'>
					新用户: ";
			...
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='复制用户'>";
		}
		...
		if ((get_request_var_post("drp_action") == "3") && (sizeof($user_array))) { /* enable */
			print "
				<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"] . "'>
						<p>当您点击 \"继续\" 已选用户将被启用.</p>
						<p><ul>$user_list</ul></p>
					</td>
				</tr>\n";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='启用用户'>";
		}
		...
		if ((get_request_var_post("drp_action") == "4") && (sizeof($user_array))) { /* disable */
			print "
				<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"] . "'>
						<p>当您点击 \"继续\" 已选用户将被禁用.</p>
						<p><ul>$user_list</ul></p>
					</td>
				</tr>\n";
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='禁用用户'>";
		}
		...
		if ((get_request_var_post("drp_action") == "5") && (sizeof($user_array))) { /* batch copy */
			...
			print "
				<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"] . "'>当您点击 \"继续\" 您将使用模板用户设置和权限覆盖已选用户?模板用户全名,密码,验放方法和启用状态将被保留,所有其它字段将被模板用户覆盖.<br><br></td>
				</tr><tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"] . "'>
						模板用户: \n";
			...
			$save_html = "<input type='button' value='取消' onClick='window.history.back()'>&nbsp;<input type='submit' value='继续' title='覆盖用户设置'>";
		}
	}else{
		print "<tr><td bgcolor='#" . $colors["form_alternate1"]. "'><span class='textError'>您必需至少选择一个用户.</span></td></tr>\n";
...
function graph_perms_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$header_label = "[编辑: " . db_fetch_cell("SELECT username FROM user_auth WHERE id = " . get_request_var("id")) . "]";
	}
	...
	<table width='100%' align='center' cellpadding="5">
		<tr>
			<td>
				<span style='font-size: 12px; font-weight: bold;'>图形策略将顺序检查以下权限直到找到一个匹配项.</span>
			</td>
		</tr>
	</table>
	<?php
	...
	html_start_box("<strong>图形权限(选择图形)</strong>", "100%", $colors["header"], "3", "center", "");
	...
	<tr bgcolor="#<?php print $colors["form_alternate1"];?>">
		<td width="50%">
			<font class="textEditTitle">默认策略</font><br>
			这个用户的默认图形策略.
		</td>
		<td align="right">
	...
	<tr>
		<td colspan="2">
			<table width="100%" cellpadding="1">
				...
				}else{ print "<tr><td><em>无图形</em></td></tr>";
				}
				?>
			</table>
		</td>
	...
	<table align='center' width='100%'>
		<tr>
			...
			<td align="right">
				&nbsp;<input type="submit" value="添加" name="add_graph_x" title="添加新图形权限">
			</td>
		</tr>
	</table>
	<br>
	...
	html_start_box("<strong>图形权限(选择主机)</strong>", "100%", $colors["header"], "3", "center", "");
	...
	<table align='center' width='100%'>
		<tr>
			...
			<td align="right">
				&nbsp;<input type="submit" value="添加" name="add_host_x" title="添加新主机权限">
			</td>
		</tr>
	</table>
	<br>
	...
	html_start_box("<strong>图形权限(选择图形模板)</strong>", "100%", $colors["header"], "3", "center", "");
	...
	<tr>
		<td colspan="2">
			<table width="100%" cellpadding="1">
				...
				}else{ print "<tr><td><em>无图形模板</em></td></tr>";
				}
				?>
			</table>
		</td>
	...
	<table align='center' width='100%'>
		<tr>
			...
			<td align="right">
				&nbsp;<input type="submit" value="添加" name="add_graph_template_x" title="添加图形模板权限">
			</td>
		</tr>
	</table>
	<br>
	...
	html_start_box("<strong>树权限</strong>", "100%", $colors["header"], "3", "center", "");
	...
	<tr>
		<td colspan="2">
			<table width="100%" cellpadding="1">
				...
				}else{ print "<tr><td><em>无树</em></td></tr>";
				}
				?>
			</table>
		</td>
	...
	<table align='center' width='100%'>
		<tr>
			...
			<td align="right">
				&nbsp;<input type="submit" value="添加" name="add_tree_x" title="添加新树权限">
			</td>
		</tr>
	</table>
	<br>
...
function user_realms_edit() {
	...
	<table width='100%' align='center' cellpadding="5">
		<tr>
			<td>
				<span style='font-size: 12px; font-weight: bold;'>功能权限控制用户可以访问Cacti的哪些功能.</span>
			</td>
		</tr>
	</table>
	<?php
	...
	print "	<tr bgcolor='#" . $colors["header"] . "'>
			<td class='textHeaderDark'><strong>功能权限</strong></td>
			<td width='1%' align='center' bgcolor='#819bc0' style='" . get_checkbox_style() . "'><input type='checkbox' style='margin: 0px;' name='all' title='全选' onClick='SelectAll(\"section\",this.checked)'></td>\n
		</tr>\n";
...
function graph_settings_edit() {
	...
	<table width='100%' align='center' cellpadding="5">
		<tr>
			<td>
				<span style='font-size: 12px; font-weight: bold;'>图形设置控制这个用户的图形应该如何显示.</span>
			</td>
		</tr>
	</table>
	<?php

	html_start_box("<strong>图形设置</strong>", "100%", $colors["header"], "3", "center", "");
		...
		<table class='tabs' width='100%' cellspacing='0' cellpadding='3' align='center'>
			<tr>
				...
				<td <?php print ((get_request_var_request("tab") == "user_realms_edit") ? "bgcolor='silver'" : "bgcolor='#DFDFDF'");?> nowrap='nowrap' width='150' align='center' class='tab'>
					<span class='textHeader'><a href='<?php print htmlspecialchars("user_admin.php?action=user_edit&tab=user_realms_edit&id=" . $_GET["id"]);?>'>功能权限</a></span>
				</td>
				<td width='1'></td>
				<td <?php print ((get_request_var_request("tab") == "graph_perms_edit") ? "bgcolor='silver'" : "bgcolor='#DFDFDF'");?> nowrap='nowrap' width='150' align='center' class='tab'>
					<span class='textHeader'><a href='<?php print htmlspecialchars("user_admin.php?action=user_edit&tab=graph_perms_edit&id=" . $_GET["id"]);?>'>图形权限</a></span>
				</td>
				<td width='1'></td>
				<td <?php print ((get_request_var_request("tab") == "graph_settings_edit") ? "bgcolor='silver'" : "bgcolor='#DFDFDF'");?> nowrap='nowrap' width='130' align='center' class='tab'>
					<span class='textHeader'><a href='<?php print htmlspecialchars("user_admin.php?action=user_edit&tab=graph_settings_edit&id=" . $_GET["id"]);?>'>图形设置</a></span>
				</td>
				<td></td>
			</tr>
		</table>
...
function user() {
	...
	html_start_box("<strong>用户管理</strong>", "100%", $colors["header"], "3", "center", "user_admin.php?action=user_edit");
			...
			<table width='100%' cellspacing='0' cellpadding='0' border='0'>
				<tr>
					<td align='left' class='textHeaderDark'>
						<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("user_admin.php?filter=" . get_request_var_request("filter") . "&page=" . (get_request_var_request("page") - 1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
					</td>\n
	...
	$display_text = array(
		...
		"realm" => array("区域", "ASC"),
		"policy_graphs" => array("默认图形策略", "ASC"),
		"dtime" => array("最后登录", "DESC"));
	...
	if (sizeof($user_list) > 0) {
		foreach ($user_list as $user) {
			if (empty($user["dtime"]) || ($user["dtime"] == "12/31/1969")) {
				...
				$last_login = strftime("%Y年%m月%d日,%H时%M分%S秒 ", strtotime($user["dtime"]));;
			}
			if ($user["enabled"] == "on") {
				$enabled = "是";
			}else{
			...
			if ($user["policy_graphs"] == "1") {
				form_selectable_cell("允许", $user["id"]);
			}else{
				form_selectable_cell("拒绝", $user["id"]);
			}
