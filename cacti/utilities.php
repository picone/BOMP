...
function utilities_view_user_log() {
	...
	html_start_box("<strong>用户登录历史</strong>", "100%", $colors["header"], "3", "center", "");
				...
				<tr>
					...
					<td width="1">
						<select name="username" onChange="applyViewLogFilterChange(document.form_userlog)">
							<option value="-1"<?php if (get_request_var_request("username") == "-1") {?> selected<?php }?>>全部</option>
							<option value="-2"<?php if (get_request_var_request("username") == "-2") {?> selected<?php }?>>已删除/不可用</option>
							<?php
					...
					<td width="1">
						<select name="result" onChange="applyViewLogFilterChange(document.form_userlog)">
							<option value="-1"<?php if (get_request_var_request("result") == '-1') {?> selected<?php }?>>全部</option>
							<option value="1"<?php if (get_request_var_request("result") == '1') {?> selected<?php }?>>成功</option>
							<option value="0"<?php if (get_request_var_request("result") == '0') {?> selected<?php }?>>失败</option>
						</select>
					</td>
					<td nowrap style='white-space: nowrap;' width="50">
						&nbsp;搜索:&nbsp;
					...
					<td nowrap style='white-space: nowrap;'>
						&nbsp;<input type="submit" name="go" value="确定" title="设置/刷新过滤器">
						<input type="submit" name="clear_x" value="复位" title="复位过滤器">
						<input type="submit" name="purge_x" value="删除" title="删除用户日志">
					</td>
				</tr>
			</table>
...
//	print $user_log_sql;
				...
				<table width='100%' cellspacing='0' cellpadding='0' border='0'>
					<tr>
						<td align='left' class='textHeaderDark'>
							<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("utilities.php?action=view_user_log&username=" . get_request_var_request("username") . "&filter=" . get_request_var_request("filter") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
						</td>\n
						<td align='center' class='textHeaderDark'>
							" . ((read_config_option("num_rows_data_source")*(get_request_var_request("page")-1))+1) . " 到 " . ((($total_rows < read_config_option("num_rows_data_source")) || ($total_rows < (read_config_option("num_rows_data_source")*get_request_var_request("page")))) ? $total_rows : (read_config_option("num_rows_data_source")*get_request_var_request("page"))) . " 行,共 $total_rows 行 [ 第 $url_page_select 页 ]
						</td>\n
	...
	$display_text = array(
		"username" => array("用户名", "ASC"),
		"full_name" => array("全名", "ASC"),
		"realm" => array("验证方法", "ASC"),
		"time" => array("日期", "ASC"),
		"result" => array("结果", "DESC"),
		"ip" => array("IP地址", "DESC"));
...
function utilities_view_logfile() {
	...
	html_start_box("<strong>日志文件过滤器</strong>", "100%", $colors["header"], "3", "center", "");
				...
				<tr>
					...
					<td width="1">
						<select name="message_type" onChange="applyViewLogFilterChange(document.form_logfile)">
							<option value="-1"<?php if (get_request_var_request("message_type") == '-1') {?> selected<?php }?>>全部</option>
							<option value="1"<?php if (get_request_var_request("message_type") == '1') {?> selected<?php }?>>状态</option>
							<option value="2"<?php if (get_request_var_request("message_type") == '2') {?> selected<?php }?>>警告</option>
							<option value="3"<?php if (get_request_var_request("message_type") == '3') {?> selected<?php }?>>错误</option>
							<option value="4"<?php if (get_request_var_request("message_type") == '4') {?> selected<?php }?>>调试</option>
							<option value="5"<?php if (get_request_var_request("message_type") == '5') {?> selected<?php }?>>SQL调用</option>
						</select>
					</td>
					<td nowrap style='white-space: nowrap;'>
						&nbsp;<input type="submit" name="go" value="确定" title="设置/刷新过滤器">
						<input type="submit" name="clear_x" value="复位" title="复位过滤器">
						<input type="submit" name="purge_x" value="删除" title="删除日志">
					</td>
				</tr>
				<tr>
					...
					<td width="1">
						<select name="reverse" onChange="applyViewLogFilterChange(document.form_logfile)">
							<option value="1"<?php if (get_request_var_request("reverse") == '1') {?> selected<?php }?>>更新的靠前</option>
							<option value="2"<?php if (get_request_var_request("reverse") == '2') {?> selected<?php }?>>更旧的靠前</option>
						</select>
					</td>
				</tr>
			</table>
	...
	if (get_request_var_request("message_type") > 0) {
		$start_string = "<strong>日志文件</strong> [总行数: " . sizeof($logcontents) . " - 显示匹配日志]";
	}else{
		$start_string = "<strong>日志文件</strong> [总行数: " . sizeof($logcontents) . " - 显示所有日志]";
	}
...
function utilities_clear_logfile() {
	...
	html_start_box("<strong>清除Cacti日志文件</strong>", "100%", $colors["header"], "1", "center", "");
	if (file_exists($logfile)) {
		if (is_writable($logfile)) {
			$timestamp = date("Y年m月d日 H:i:s A");
			$log_fh = fopen($logfile, "w");
			fwrite($log_fh, $timestamp . " - WEB接口:从WEB管理接口清除Cacti日志\n");
			fclose($log_fh);
			print "<tr><td>Cacti日志文件已清除</td></tr>";
		}else{
			print "<tr><td><font color='red'><b>错误:无法清除日志,没有写入权限.<b></font></td></tr>";
		}
	}else{
		print "<tr><td><font color='red'><b>错误:无法清除日志,日志文件不存在.</b></font></td></tr>";
	}
	html_end_box();
}
...
function utilities_view_snmp_cache() {
	...
	html_start_box("<strong>SNMP缓存对象</strong>", "100%", $colors["header"], "3", "center", "");
				...
				<tr>
					...
					<td width="1">
						<select name="host_id" onChange="applyViewSNMPFilterChange(document.form_snmpcache)">
							<option value="-1"<?php if (get_request_var_request("host_id") == "-1") {?> selected<?php }?>>任意</option>
							<option value="0"<?php if (get_request_var_request("host_id") == "0") {?> selected<?php }?>>无</option>
							<?php
					...
					<td width="1">
						<select name="snmp_query_id" onChange="applyViewSNMPFilterChange(document.form_snmpcache)">
							<option value="-1"<?php if (get_request_var_request("host_id") == "-1") {?> selected<?php }?>>全部</option>
							<?php
					...
					<td nowrap style='white-space: nowrap;'>
						&nbsp;<input type="submit" name="go" value="点定" title="设置/刷新过滤器">
...
//	print $snmp_cache_sql;
	...
	$nav = "<tr bgcolor='#" . $colors["header"] . "'>
			<td colspan='7'>
				<table width='100%' cellspacing='0' cellpadding='0' border='0'>
					<tr>
						<td align='left' class='textHeaderDark'>
							<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("utilities.php?action=view_snmp_cache&host_id=" . get_request_var_request("host_id") . "&filter=" . get_request_var_request("filter") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
						</td>\n
						<td align='center' class='textHeaderDark'>
							" . ((read_config_option("num_rows_data_source")*(get_request_var_request("page")-1))+1) . " 到 " . ((($total_rows < read_config_option("num_rows_data_source")) || ($total_rows < (read_config_option("num_rows_data_source")*get_request_var_request("page")))) ? $total_rows : (read_config_option("num_rows_data_source")*get_request_var_request("page"))) . " 行,共 $total_rows 行 [ 第 $url_page_select 页 ]
						</td>\n
	...
	foreach ($snmp_cache as $item) {
		...
		?>
		<td>
			主机: <?php print (strlen(get_request_var_request("filter")) ? (eregi_replace("(" . preg_quote(get_request_var_request("filter")) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", $item["description"])) : $item["description"]);?>
			, SNMP查询: <?php print (strlen(get_request_var_request("filter")) ? (eregi_replace("(" . preg_quote(get_request_var_request("filter")) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", $item["name"])) : $item["name"]);?>
		</td>
		</tr>
		<?php
		form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],$i);
		?>
		<td>
			索引: <?php print $item["snmp_index"];?>
			, 字段名称: <?php print (strlen(get_request_var_request("filter")) ? (eregi_replace("(" . preg_quote(get_request_var_request("filter")) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", $item["field_name"])) : $item["field_name"]);?>
			, 字段值: <?php print (strlen(get_request_var_request("filter")) ? (eregi_replace("(" . preg_quote(get_request_var_request("filter")) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", $item["field_value"])) : $item["field_value"]);?>
		</td>
		</tr>
		<?php
...
function utilities_view_poller_cache() {
	...
	html_start_box("<strong>采集器缓存对象</strong>", "100%", $colors["header"], "3", "center", "");
				...
				<tr>
					...
					<td width="1">
						<select name="poller_action" onChange="applyPItemFilterChange(document.form_pollercache)">
							<option value="-1"<?php if (get_request_var_request("poller_action") == '-1') {?> selected<?php }?>>全部</option>
							<option value="0"<?php if (get_request_var_request("poller_action") == '0') {?> selected<?php }?>>SNMP</option>
							<option value="1"<?php if (get_request_var_request("poller_action") == '1') {?> selected<?php }?>>脚本</option>
							<option value="2"<?php if (get_request_var_request("poller_action") == '2') {?> selected<?php }?>>脚本服务器</option>
						</select>
					</td>
					<td nowrap style='white-space: nowrap;' width="50">
						&nbsp;搜索:&nbsp;
...
//	print $poller_sql;
	...
	$nav = "<tr bgcolor='#" . $colors["header"] . "'>
			<td colspan='7'>
				<table width='100%' cellspacing='0' cellpadding='0' border='0'>
					<tr>
						<td align='left' class='textHeaderDark'>
							<strong>&lt;&lt; "; if (get_request_var_request("page") > 1) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("utilities.php?action=view_poller_cache&host_id=" . get_request_var_request("host_id") . "&poller_action=" . get_request_var_request("poller_action") . "&page=" . (get_request_var_request("page")-1)) . "'>"; } $nav .= "上一页"; if (get_request_var_request("page") > 1) { $nav .= "</a>"; } $nav .= "</strong>
						</td>\n
						<td align='center' class='textHeaderDark'>
							" . ((read_config_option("num_rows_data_source")*(get_request_var_request("page")-1))+1) . " 到 " . ((($total_rows < read_config_option("num_rows_data_source")) || ($total_rows < (read_config_option("num_rows_data_source")*get_request_var_request("page")))) ? $total_rows : (read_config_option("num_rows_data_source")*get_request_var_request("page"))) . " 行,共 $total_rows 行 [ 第 $url_page_select 页]
						</td>\n
						<td align='right' class='textHeaderDark'>
							<strong>"; if ((get_request_var_request("page") * read_config_option("num_rows_data_source")) < $total_rows) { $nav .= "<a class='linkOverDark' href='" . htmlspecialchars("utilities.php?action=view_poller_cache&host_id=" . get_request_var_request("host_id") . "&poller_action=" . get_request_var_request("poller_action") . "&page=" . (get_request_var_request("page")+1)) . "'>"; } $nav .= "下一页"; if ((get_request_var_request("page") * read_config_option("num_rows_data_source")) < $total_rows) { $nav .= "</a>"; } $nav .= " &gt;&gt;</strong>
						</td>\n
					</tr>
				</table>
			</td>
	...
	$display_text = array(
		"data_template_data.name_cache" => array("数据源名称", "ASC"),
		"nosort" => array("详细", "ASC"));
	...
	foreach ($poller_cache as $item) {
		form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],$i);
			?>
			<td width="375">
				<a class="linkEditMain" href="<?php print htmlspecialchars("data_sources.php?action=ds_edit&id=" . $item["local_data_id"]);?>"><?php print (strlen(get_request_var_request("filter")) ? eregi_replace("(" . preg_quote(get_request_var_request("filter")) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", $item["name_cache"]):$item["name_cache"]);?></a>
			</td>
			...
			if ($item["action"] == 0) {
				if ($item["snmp_version"] != 3) {
					$details =
						"SNMP版本: " . $item["snmp_version"] . ", " .
						"团体名称: " . $item["snmp_community"] . ", " .
					...
					$details =
						"SNMP版本: " . $item["snmp_version"] . ", " .
						"用户名: " . $item["snmp_username"] . ", OID: " . $item["arg1"];
				}
			}elseif ($item["action"] == 1) {
					$details = "脚本: " . (strlen(get_request_var_request("filter")) ? (eregi_replace("(" . preg_quote(get_request_var_request("filter")) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", $item["arg1"])) : $item["arg1"]);
			}else{
					$details = "脚本服务器: " . (strlen(get_request_var_request("filter")) ? (eregi_replace("(" . preg_quote(get_request_var_request("filter")) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", $item["arg1"])) : $item["arg1"]);
			}
...
function utilities() {
	...
	html_start_box("<strong>Cacti系统工具</strong>", "100%", $colors["header"], "3", "center", "");
	...
	<?php html_header(array("日志管理"), 2);?>

	<tr bgcolor="#<?php print $colors["form_alternate1"];?>">
		<td class="textArea">
			<a href='<?php print htmlspecialchars("utilities.php?action=view_logfile");?>'>查看Cacti日志</a>
		</td>
		<td class="textArea">
			Cacit日志可以有效的帮助您找到问题的原因.
		</td>
	</tr>
	<tr bgcolor="#<?php print $colors["form_alternate1"];?>">
		<td class="textArea">
			<a href='<?php print htmlspecialchars("utilities.php?action=view_user_log");?>'>查看用户日志</a>
		</td>
		<td class="textArea">
			允许管理员查看,过滤和导出用户日志.
		</td>
	...
	<?php html_header(array("采集器缓存管理"), 2); ?>

	<tr bgcolor="#<?php print $colors["form_alternate1"];?>">
		<td class="textArea">
			<a href='<?php print htmlspecialchars("utilities.php?action=view_poller_cache");?>'>查看采集器缓存</a>
		</td>
		<td class="textArea">
			采集器每次运行都会把新的数据添加到这个数据库,每次绘图也会使用这个数据库.
		</td>
	</tr>
	<tr bgcolor="#<?php print $colors["form_alternate2"];?>">
		<td class="textArea">
			<a href='<?php print htmlspecialchars("utilities.php?action=view_snmp_cache");?>'>查看SNMP缓存</a>
		</td>
		<td class="textArea">
			SNMP缓存保存了SNMP查询时采集到的信息.当Cacti采集新数据的时候会使用这个缓存来确定需要查询的OID.
		</td>
	</tr>
	<tr bgcolor="#<?php print $colors["form_alternate1"];?>">
		<td class="textArea">
			<a href='<?php print htmlspecialchars("utilities.php?action=clear_poller_cache");?>'>重建采集器缓存</a>
		</td>
		<td class="textArea">
			这个功能将会清除并重新产生采集器缓存.某些时候主机或数据不同步,使用这个功能可以纠正错误.当绘图出现问题时可以尝试使用该功能.
		</td>
	</tr>
